...
pub struct PersonalTableLegendsArceus(Vec<PersonalInfoLegendsArceus>);

impl PersonalTableLegendsArceus {
    pub fn from_pkl_bytes(bytes: &[u8]) -> Self {
        let count = bytes.len() / LA_ENTRY_SIZE;
        let mut entries = Vec::<PersonalInfoLegendsArceus>::with_capacity(count);
        for i in 0..count {
            let offset = i * LA_ENTRY_SIZE;
            entries.push(PersonalInfoLegendsArceus::from_pkl_bytes(
                bytes[offset..offset + LA_ENTRY_SIZE]
                    .try_into()
                    .expect("incorrect slice length for legends arceus personal table"),
            ));
        }
        Self(entries)
    }
    ...
    pub fn get_personal_info(
        ...
        national_dex: u16,
        forme_index: u16,
    ) -> Option<&PersonalInfoLegendsArceus> {
    ...
    pub fn get_form_stats(&self, national_dex: u16, forme_index: u16) -> Option<Stats8> {
        self.get_personal_info(national_dex, forme_index)
            .map(PersonalInfoLegendsArceus::stats)
    }
}

impl PersonalTable for PersonalTableLegendsArceus {
    fn get_types(&self, national_dex: u16, forme_index: u16) -> Option<(PkmType, PkmType)> {
        self.get_personal_info(national_dex, forme_index)
            .map(PersonalInfoLegendsArceus::types)
    }
...
impl MetadataTableLegendsArceus {
