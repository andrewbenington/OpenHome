use crate::items::ItemMetadataPastGen;
use crate::items::types_extra::ItemUnbound;

pub const ITEM_MAX_UNBOUND: usize = 728;

impl ItemUnbound {
    pub fn from_modern_index(modern_index: u16) -> Option<Self> {
        match modern_index {
            1 => Self::new(1),
            2 => Self::new(2),
            3 => Self::new(3),
            4 => Self::new(4),
            5 => Self::new(5),
            6 => Self::new(6),
            7 => Self::new(7),
            8 => Self::new(8),
            9 => Self::new(9),
            10 => Self::new(10),
            11 => Self::new(11),
            12 => Self::new(12),
            13 => Self::new(60),
            14 => Self::new(61),
            15 => Self::new(62),
            16 => Self::new(53),
            17 => Self::new(13),
            18 => Self::new(14),
            19 => Self::new(15),
            20 => Self::new(16),
            21 => Self::new(17),
            22 => Self::new(18),
            23 => Self::new(19),
            24 => Self::new(20),
            25 => Self::new(21),
            26 => Self::new(22),
            27 => Self::new(23),
            28 => Self::new(24),
            29 => Self::new(25),
            30 => Self::new(26),
            31 => Self::new(27),
            32 => Self::new(28),
            33 => Self::new(29),
            34 => Self::new(30),
            35 => Self::new(31),
            36 => Self::new(32),
            37 => Self::new(33),
            38 => Self::new(34),
            39 => Self::new(35),
            40 => Self::new(36),
            41 => Self::new(37),
            42 => Self::new(38),
            43 => Self::new(44),
            44 => Self::new(45),
            45 => Self::new(63),
            46 => Self::new(64),
            47 => Self::new(65),
            48 => Self::new(66),
            49 => Self::new(67),
            50 => Self::new(68),
            51 => Self::new(69),
            52 => Self::new(70),
            53 => Self::new(71),
            54 => Self::new(520),
            56 => Self::new(74),
            57 => Self::new(75),
            58 => Self::new(76),
            59 => Self::new(77),
            60 => Self::new(78),
            62 => Self::new(82),
            63 => Self::new(80),
            64 => Self::new(81),
            65 => Self::new(39),
            66 => Self::new(40),
            67 => Self::new(41),
            68 => Self::new(42),
            69 => Self::new(43),
            70 => Self::new(46),
            71 => Self::new(47),
            72 => Self::new(48),
            73 => Self::new(49),
            74 => Self::new(50),
            75 => Self::new(51),
            76 => Self::new(83),
            77 => Self::new(84),
            78 => Self::new(85),
            79 => Self::new(86),
            80 => Self::new(93),
            81 => Self::new(94),
            82 => Self::new(95),
            83 => Self::new(96),
            84 => Self::new(97),
            85 => Self::new(98),
            86 => Self::new(103),
            87 => Self::new(104),
            88 => Self::new(106),
            89 => Self::new(107),
            90 => Self::new(108),
            91 => Self::new(109),
            92 => Self::new(110),
            93 => Self::new(111),
            94 => Self::new(72),
            99 => Self::new(286),
            100 => Self::new(287),
            101 => Self::new(357),
            102 => Self::new(358),
            103 => Self::new(354),
            104 => Self::new(522),
            105 => Self::new(521),
            106 => Self::new(114),
            107 => Self::new(101),
            108 => Self::new(99),
            109 => Self::new(100),
            110 => Self::new(102),
            111 => Self::new(251),
            112 => Self::new(534),
            116 => Self::new(635),
            117 => Self::new(636),
            118 => Self::new(634),
            119 => Self::new(637),
            135 => Self::new(532),
            136 => Self::new(533),
            149 => Self::new(133),
            150 => Self::new(134),
            151 => Self::new(135),
            152 => Self::new(136),
            153 => Self::new(137),
            154 => Self::new(138),
            155 => Self::new(139),
            156 => Self::new(140),
            157 => Self::new(141),
            158 => Self::new(142),
            159 => Self::new(143),
            160 => Self::new(144),
            161 => Self::new(145),
            162 => Self::new(146),
            163 => Self::new(147),
            164 => Self::new(148),
            165 => Self::new(149),
            166 => Self::new(150),
            167 => Self::new(151),
            168 => Self::new(152),
            169 => Self::new(153),
            170 => Self::new(154),
            171 => Self::new(155),
            172 => Self::new(156),
            173 => Self::new(157),
            174 => Self::new(158),
            175 => Self::new(159),
            176 => Self::new(160),
            177 => Self::new(161),
            178 => Self::new(162),
            179 => Self::new(163),
            180 => Self::new(164),
            181 => Self::new(165),
            182 => Self::new(166),
            183 => Self::new(167),
            184 => Self::new(539),
            185 => Self::new(540),
            186 => Self::new(541),
            187 => Self::new(542),
            188 => Self::new(543),
            189 => Self::new(544),
            190 => Self::new(545),
            191 => Self::new(546),
            192 => Self::new(547),
            193 => Self::new(548),
            194 => Self::new(549),
            195 => Self::new(550),
            196 => Self::new(551),
            197 => Self::new(552),
            198 => Self::new(553),
            199 => Self::new(554),
            200 => Self::new(555),
            201 => Self::new(168),
            202 => Self::new(169),
            203 => Self::new(170),
            204 => Self::new(171),
            205 => Self::new(172),
            206 => Self::new(173),
            208 => Self::new(118),
            209 => Self::new(556),
            210 => Self::new(557),
            211 => Self::new(558),
            212 => Self::new(559),
            213 => Self::new(179),
            214 => Self::new(180),
            215 => Self::new(181),
            216 => Self::new(182),
            217 => Self::new(183),
            218 => Self::new(184),
            219 => Self::new(185),
            220 => Self::new(176),
            221 => Self::new(187),
            222 => Self::new(188),
            223 => Self::new(189),
            224 => Self::new(190),
            225 => Self::new(191),
            226 => Self::new(192),
            227 => Self::new(193),
            228 => Self::new(194),
            229 => Self::new(195),
            230 => Self::new(196),
            231 => Self::new(197),
            232 => Self::new(198),
            233 => Self::new(199),
            234 => Self::new(200),
            235 => Self::new(201),
            236 => Self::new(202),
            237 => Self::new(203),
            238 => Self::new(204),
            239 => Self::new(205),
            240 => Self::new(206),
            241 => Self::new(207),
            242 => Self::new(208),
            243 => Self::new(209),
            244 => Self::new(210),
            245 => Self::new(211),
            246 => Self::new(212),
            247 => Self::new(213),
            248 => Self::new(214),
            249 => Self::new(215),
            250 => Self::new(216),
            251 => Self::new(217),
            252 => Self::new(218),
            253 => Self::new(219),
            254 => Self::new(220),
            255 => Self::new(221),
            256 => Self::new(222),
            257 => Self::new(223),
            258 => Self::new(224),
            259 => Self::new(225),
            265 => Self::new(237),
            266 => Self::new(577),
            267 => Self::new(585),
            268 => Self::new(235),
            269 => Self::new(90),
            270 => Self::new(55),
            271 => Self::new(236),
            272 => Self::new(56),
            273 => Self::new(57),
            274 => Self::new(575),
            275 => Self::new(570),
            276 => Self::new(238),
            277 => Self::new(576),
            278 => Self::new(572),
            279 => Self::new(573),
            280 => Self::new(239),
            281 => Self::new(58),
            282 => Self::new(243),
            283 => Self::new(240),
            284 => Self::new(242),
            285 => Self::new(241),
            286 => Self::new(571),
            287 => Self::new(178),
            288 => Self::new(583),
            289 => Self::new(244),
            290 => Self::new(245),
            291 => Self::new(246),
            292 => Self::new(247),
            293 => Self::new(248),
            294 => Self::new(249),
            295 => Self::new(581),
            296 => Self::new(250),
            297 => Self::new(177),
            298 => Self::new(504),
            299 => Self::new(505),
            300 => Self::new(507),
            301 => Self::new(506),
            302 => Self::new(509),
            303 => Self::new(496),
            304 => Self::new(498),
            305 => Self::new(499),
            306 => Self::new(497),
            307 => Self::new(508),
            308 => Self::new(501),
            309 => Self::new(500),
            310 => Self::new(502),
            311 => Self::new(510),
            312 => Self::new(511),
            313 => Self::new(503),
            314 => Self::new(254),
            315 => Self::new(256),
            316 => Self::new(253),
            317 => Self::new(258),
            318 => Self::new(257),
            319 => Self::new(252),
            320 => Self::new(255),
            321 => Self::new(112),
            322 => Self::new(234),
            323 => Self::new(113),
            324 => Self::new(233),
            325 => Self::new(232),
            326 => Self::new(230),
            327 => Self::new(231),
            328 => Self::new(289),
            329 => Self::new(290),
            330 => Self::new(291),
            331 => Self::new(292),
            332 => Self::new(293),
            333 => Self::new(294),
            334 => Self::new(295),
            335 => Self::new(296),
            336 => Self::new(297),
            337 => Self::new(298),
            338 => Self::new(299),
            339 => Self::new(300),
            340 => Self::new(301),
            341 => Self::new(302),
            342 => Self::new(303),
            343 => Self::new(304),
            344 => Self::new(305),
            345 => Self::new(306),
            346 => Self::new(307),
            347 => Self::new(308),
            348 => Self::new(309),
            349 => Self::new(310),
            350 => Self::new(311),
            351 => Self::new(312),
            352 => Self::new(313),
            353 => Self::new(314),
            354 => Self::new(315),
            355 => Self::new(316),
            356 => Self::new(317),
            357 => Self::new(318),
            358 => Self::new(319),
            359 => Self::new(320),
            360 => Self::new(321),
            361 => Self::new(322),
            362 => Self::new(323),
            363 => Self::new(324),
            364 => Self::new(325),
            365 => Self::new(326),
            366 => Self::new(327),
            367 => Self::new(328),
            368 => Self::new(329),
            369 => Self::new(330),
            370 => Self::new(331),
            371 => Self::new(332),
            372 => Self::new(333),
            373 => Self::new(334),
            374 => Self::new(335),
            375 => Self::new(336),
            376 => Self::new(337),
            377 => Self::new(338),
            378 => Self::new(339),
            379 => Self::new(340),
            380 => Self::new(341),
            381 => Self::new(342),
            382 => Self::new(343),
            383 => Self::new(344),
            384 => Self::new(345),
            385 => Self::new(346),
            386 => Self::new(375),
            387 => Self::new(376),
            388 => Self::new(377),
            389 => Self::new(378),
            390 => Self::new(379),
            391 => Self::new(380),
            392 => Self::new(381),
            393 => Self::new(382),
            394 => Self::new(383),
            395 => Self::new(384),
            396 => Self::new(385),
            397 => Self::new(386),
            398 => Self::new(387),
            399 => Self::new(388),
            400 => Self::new(389),
            401 => Self::new(390),
            402 => Self::new(391),
            403 => Self::new(392),
            404 => Self::new(393),
            405 => Self::new(394),
            406 => Self::new(395),
            407 => Self::new(396),
            408 => Self::new(397),
            409 => Self::new(398),
            410 => Self::new(399),
            411 => Self::new(400),
            412 => Self::new(401),
            413 => Self::new(402),
            414 => Self::new(403),
            415 => Self::new(404),
            416 => Self::new(405),
            417 => Self::new(406),
            418 => Self::new(407),
            419 => Self::new(408),
            420 => Self::new(437),
            421 => Self::new(438),
            422 => Self::new(439),
            423 => Self::new(440),
            424 => Self::new(441),
            425 => Self::new(442),
            435 => Self::new(270),
            442 => Self::new(361),
            444 => Self::new(260),
            445 => Self::new(262),
            446 => Self::new(263),
            447 => Self::new(264),
            453 => Self::new(373),
            454 => Self::new(374),
            456 => Self::new(265),
            458 => Self::new(92),
            459 => Self::new(349),
            466 => Self::new(535),
            471 => Self::new(261),
            476 => Self::new(271),
            482 => Self::new(656),
            483 => Self::new(655),
            492 => Self::new(622),
            493 => Self::new(623),
            494 => Self::new(624),
            495 => Self::new(625),
            496 => Self::new(626),
            497 => Self::new(627),
            498 => Self::new(628),
            499 => Self::new(629),
            500 => Self::new(52),
            532 => Self::new(350),
            534 => Self::new(276),
            535 => Self::new(277),
            537 => Self::new(115),
            538 => Self::new(495),
            539 => Self::new(569),
            540 => Self::new(226),
            541 => Self::new(564),
            542 => Self::new(579),
            543 => Self::new(580),
            544 => Self::new(566),
            545 => Self::new(563),
            546 => Self::new(567),
            547 => Self::new(568),
            548 => Self::new(599),
            549 => Self::new(600),
            550 => Self::new(602),
            551 => Self::new(601),
            552 => Self::new(604),
            553 => Self::new(591),
            554 => Self::new(593),
            555 => Self::new(594),
            556 => Self::new(592),
            557 => Self::new(603),
            558 => Self::new(596),
            559 => Self::new(595),
            560 => Self::new(597),
            561 => Self::new(605),
            562 => Self::new(606),
            563 => Self::new(598),
            564 => Self::new(590),
            565 => Self::new(609),
            566 => Self::new(610),
            567 => Self::new(611),
            568 => Self::new(612),
            569 => Self::new(613),
            570 => Self::new(614),
            572 => Self::new(523),
            573 => Self::new(524),
            576 => Self::new(631),
            580 => Self::new(514),
            581 => Self::new(229),
            582 => Self::new(228),
            583 => Self::new(513),
            584 => Self::new(663),
            585 => Self::new(664),
            586 => Self::new(665),
            587 => Self::new(666),
            588 => Self::new(667),
            589 => Self::new(668),
            590 => Self::new(669),
            591 => Self::new(516),
            616 => Self::new(661),
            617 => Self::new(662),
            618 => Self::new(409),
            619 => Self::new(410),
            620 => Self::new(411),
            628 => Self::new(536),
            631 => Self::new(632),
            632 => Self::new(633),
            634 => Self::new(266),
            638 => Self::new(537),
            639 => Self::new(227),
            640 => Self::new(494),
            644 => Self::new(512),
            645 => Self::new(608),
            646 => Self::new(117),
            647 => Self::new(116),
            648 => Self::new(574),
            649 => Self::new(582),
            650 => Self::new(493),
            656 => Self::new(454),
            657 => Self::new(470),
            658 => Self::new(461),
            659 => Self::new(446),
            660 => Self::new(447),
            661 => Self::new(449),
            662 => Self::new(459),
            663 => Self::new(460),
            664 => Self::new(468),
            665 => Self::new(474),
            666 => Self::new(465),
            667 => Self::new(473),
            668 => Self::new(479),
            669 => Self::new(466),
            670 => Self::new(463),
            671 => Self::new(456),
            672 => Self::new(458),
            673 => Self::new(488),
            674 => Self::new(489),
            675 => Self::new(455),
            676 => Self::new(457),
            677 => Self::new(480),
            678 => Self::new(448),
            679 => Self::new(452),
            680 => Self::new(464),
            681 => Self::new(472),
            682 => Self::new(475),
            683 => Self::new(487),
            684 => Self::new(484),
            685 => Self::new(485),
            686 => Self::new(560),
            687 => Self::new(561),
            688 => Self::new(562),
            690 => Self::new(412),
            691 => Self::new(413),
            692 => Self::new(414),
            693 => Self::new(415),
            694 => Self::new(416),
            696 => Self::new(353),
            708 => Self::new(517),
            709 => Self::new(519),
            710 => Self::new(525),
            711 => Self::new(526),
            715 => Self::new(607),
            716 => Self::new(528),
            718 => Self::new(259),
            725 => Self::new(274),
            726 => Self::new(275),
            728 => Self::new(279),
            729 => Self::new(280),
            733 => Self::new(284),
            735 => Self::new(288),
            737 => Self::new(443),
            744 => Self::new(529),
            745 => Self::new(531),
            749 => Self::new(120),
            750 => Self::new(530),
            752 => Self::new(469),
            753 => Self::new(467),
            754 => Self::new(471),
            755 => Self::new(478),
            756 => Self::new(490),
            757 => Self::new(491),
            758 => Self::new(483),
            759 => Self::new(476),
            760 => Self::new(453),
            761 => Self::new(462),
            762 => Self::new(451),
            763 => Self::new(481),
            764 => Self::new(492),
            765 => Self::new(538),
            766 => Self::new(119),
            767 => Self::new(477),
            768 => Self::new(486),
            769 => Self::new(482),
            770 => Self::new(450),
            776 => Self::new(670),
            777 => Self::new(679),
            778 => Self::new(680),
            779 => Self::new(682),
            780 => Self::new(681),
            781 => Self::new(684),
            782 => Self::new(671),
            783 => Self::new(673),
            784 => Self::new(674),
            785 => Self::new(672),
            786 => Self::new(683),
            787 => Self::new(676),
            788 => Self::new(675),
            789 => Self::new(677),
            790 => Self::new(685),
            791 => Self::new(686),
            792 => Self::new(678),
            793 => Self::new(687),
            794 => Self::new(698),
            795 => Self::new(616),
            796 => Self::new(617),
            798 => Self::new(689),
            799 => Self::new(691),
            800 => Self::new(700),
            801 => Self::new(703),
            802 => Self::new(695),
            803 => Self::new(688),
            804 => Self::new(701),
            805 => Self::new(690),
            806 => Self::new(696),
            846 => Self::new(565),
            849 => Self::new(105),
            851 => Self::new(630),
            852 => Self::new(515),
            853 => Self::new(618),
            854 => Self::new(619),
            855 => Self::new(620),
            856 => Self::new(621),
            857 => Self::new(659),
            858 => Self::new(660),
            879 => Self::new(584),
            880 => Self::new(578),
            881 => Self::new(586),
            882 => Self::new(589),
            883 => Self::new(588),
            884 => Self::new(587),
            904 => Self::new(638),
            905 => Self::new(639),
            907 => Self::new(641),
            908 => Self::new(642),
            909 => Self::new(643),
            910 => Self::new(644),
            911 => Self::new(645),
            912 => Self::new(646),
            913 => Self::new(647),
            914 => Self::new(648),
            915 => Self::new(649),
            916 => Self::new(650),
            917 => Self::new(651),
            918 => Self::new(652),
            919 => Self::new(653),
            920 => Self::new(654),
            921 => Self::new(702),
            922 => Self::new(693),
            923 => Self::new(445),
            924 => Self::new(697),
            925 => Self::new(694),
            926 => Self::new(692),
            943 => Self::new(657),
            944 => Self::new(658),
            1077 => Self::new(268),
            1103 => Self::new(710),
            1104 => Self::new(711),
            1109 => Self::new(716),
            1110 => Self::new(718),
            1111 => Self::new(717),
            1112 => Self::new(719),
            1113 => Self::new(720),
            1114 => Self::new(722),
            1115 => Self::new(721),
            1116 => Self::new(712),
            1117 => Self::new(713),
            1118 => Self::new(709),
            1119 => Self::new(704),
            1120 => Self::new(707),
            1121 => Self::new(706),
            1122 => Self::new(705),
            1123 => Self::new(708),
            1252 => Self::new(88),
            1253 => Self::new(714),
            1254 => Self::new(715),
            1267 => Self::new(371),
            1582 => Self::new(723),
            1590 => Self::new(725),
            1592 => Self::new(724),
            1611 => Self::new(91),
            1691 => Self::new(726),
            1692 => Self::new(727),
            2161 => Self::new(417),
            2162 => Self::new(418),
            2163 => Self::new(419),
            2164 => Self::new(420),
            2165 => Self::new(421),
            2166 => Self::new(422),
            2167 => Self::new(423),
            2168 => Self::new(424),
            2169 => Self::new(425),
            2170 => Self::new(426),
            2171 => Self::new(427),
            2172 => Self::new(428),
            2173 => Self::new(429),
            2174 => Self::new(430),
            2175 => Self::new(431),
            2176 => Self::new(432),
            2177 => Self::new(433),
            2178 => Self::new(434),
            2179 => Self::new(435),
            2180 => Self::new(436),
            _ => None,
        }
    }
}

pub static ALL_ITEMS_UNBOUND: [&ItemMetadataPastGen; ITEM_MAX_UNBOUND] = [
    &ItemMetadataPastGen {
        id: 1,
        modern_id: Some(1),
        name: "Master Ball",
    },
    &ItemMetadataPastGen {
        id: 2,
        modern_id: Some(2),
        name: "Ultra Ball",
    },
    &ItemMetadataPastGen {
        id: 3,
        modern_id: Some(3),
        name: "Great Ball",
    },
    &ItemMetadataPastGen {
        id: 4,
        modern_id: Some(4),
        name: "Poke Ball",
    },
    &ItemMetadataPastGen {
        id: 5,
        modern_id: Some(5),
        name: "Safari Ball",
    },
    &ItemMetadataPastGen {
        id: 6,
        modern_id: Some(6),
        name: "Net Ball",
    },
    &ItemMetadataPastGen {
        id: 7,
        modern_id: Some(7),
        name: "Dive Ball",
    },
    &ItemMetadataPastGen {
        id: 8,
        modern_id: Some(8),
        name: "Nest Ball",
    },
    &ItemMetadataPastGen {
        id: 9,
        modern_id: Some(9),
        name: "Repeat Ball",
    },
    &ItemMetadataPastGen {
        id: 10,
        modern_id: Some(10),
        name: "Timer Ball",
    },
    &ItemMetadataPastGen {
        id: 11,
        modern_id: Some(11),
        name: "Luxury Ball",
    },
    &ItemMetadataPastGen {
        id: 12,
        modern_id: Some(12),
        name: "Premier Ball",
    },
    &ItemMetadataPastGen {
        id: 13,
        modern_id: Some(17),
        name: "Potion",
    },
    &ItemMetadataPastGen {
        id: 14,
        modern_id: Some(18),
        name: "Antidote",
    },
    &ItemMetadataPastGen {
        id: 15,
        modern_id: Some(19),
        name: "Burn Heal",
    },
    &ItemMetadataPastGen {
        id: 16,
        modern_id: Some(20),
        name: "Ice Heal",
    },
    &ItemMetadataPastGen {
        id: 17,
        modern_id: Some(21),
        name: "Awakening",
    },
    &ItemMetadataPastGen {
        id: 18,
        modern_id: Some(22),
        name: "Parlyz Heal",
    },
    &ItemMetadataPastGen {
        id: 19,
        modern_id: Some(23),
        name: "Full Restore",
    },
    &ItemMetadataPastGen {
        id: 20,
        modern_id: Some(24),
        name: "Max Potion",
    },
    &ItemMetadataPastGen {
        id: 21,
        modern_id: Some(25),
        name: "Hyper Potion",
    },
    &ItemMetadataPastGen {
        id: 22,
        modern_id: Some(26),
        name: "Super Potion",
    },
    &ItemMetadataPastGen {
        id: 23,
        modern_id: Some(27),
        name: "Full Heal",
    },
    &ItemMetadataPastGen {
        id: 24,
        modern_id: Some(28),
        name: "Revive",
    },
    &ItemMetadataPastGen {
        id: 25,
        modern_id: Some(29),
        name: "Max Revive",
    },
    &ItemMetadataPastGen {
        id: 26,
        modern_id: Some(30),
        name: "Fresh Water",
    },
    &ItemMetadataPastGen {
        id: 27,
        modern_id: Some(31),
        name: "Soda Pop",
    },
    &ItemMetadataPastGen {
        id: 28,
        modern_id: Some(32),
        name: "Lemonade",
    },
    &ItemMetadataPastGen {
        id: 29,
        modern_id: Some(33),
        name: "Moomoo Milk",
    },
    &ItemMetadataPastGen {
        id: 30,
        modern_id: Some(34),
        name: "Energy Powder",
    },
    &ItemMetadataPastGen {
        id: 31,
        modern_id: Some(35),
        name: "Energy Root",
    },
    &ItemMetadataPastGen {
        id: 32,
        modern_id: Some(36),
        name: "Heal Powder",
    },
    &ItemMetadataPastGen {
        id: 33,
        modern_id: Some(37),
        name: "Revival Herb",
    },
    &ItemMetadataPastGen {
        id: 34,
        modern_id: Some(38),
        name: "Ether",
    },
    &ItemMetadataPastGen {
        id: 35,
        modern_id: Some(39),
        name: "Max Ether",
    },
    &ItemMetadataPastGen {
        id: 36,
        modern_id: Some(40),
        name: "Elixir",
    },
    &ItemMetadataPastGen {
        id: 37,
        modern_id: Some(41),
        name: "Max Elixir",
    },
    &ItemMetadataPastGen {
        id: 38,
        modern_id: Some(42),
        name: "Lava Cookie",
    },
    &ItemMetadataPastGen {
        id: 39,
        modern_id: Some(65),
        name: "Blue Flute",
    },
    &ItemMetadataPastGen {
        id: 40,
        modern_id: Some(66),
        name: "Yellow Flute",
    },
    &ItemMetadataPastGen {
        id: 41,
        modern_id: Some(67),
        name: "Red Flute",
    },
    &ItemMetadataPastGen {
        id: 42,
        modern_id: Some(68),
        name: "Black Flute",
    },
    &ItemMetadataPastGen {
        id: 43,
        modern_id: Some(69),
        name: "White Flute",
    },
    &ItemMetadataPastGen {
        id: 44,
        modern_id: Some(43),
        name: "Berry Juice",
    },
    &ItemMetadataPastGen {
        id: 45,
        modern_id: Some(44),
        name: "Sacred Ash",
    },
    &ItemMetadataPastGen {
        id: 46,
        modern_id: Some(70),
        name: "Shoal Salt",
    },
    &ItemMetadataPastGen {
        id: 47,
        modern_id: Some(71),
        name: "Shoal Shell",
    },
    &ItemMetadataPastGen {
        id: 48,
        modern_id: Some(72),
        name: "Red Shard",
    },
    &ItemMetadataPastGen {
        id: 49,
        modern_id: Some(73),
        name: "Blue Shard",
    },
    &ItemMetadataPastGen {
        id: 50,
        modern_id: Some(74),
        name: "Yellow Shard",
    },
    &ItemMetadataPastGen {
        id: 51,
        modern_id: Some(75),
        name: "Green Shard",
    },
    &ItemMetadataPastGen {
        id: 52,
        modern_id: Some(500),
        name: "Park Ball",
    },
    &ItemMetadataPastGen {
        id: 53,
        modern_id: Some(16),
        name: "Cherish Ball",
    },
    &ItemMetadataPastGen {
        id: 54,
        modern_id: None,
        name: "",
    },
    &ItemMetadataPastGen {
        id: 55,
        modern_id: Some(270),
        name: "Life Orb",
    },
    &ItemMetadataPastGen {
        id: 56,
        modern_id: Some(272),
        name: "Toxic Orb",
    },
    &ItemMetadataPastGen {
        id: 57,
        modern_id: Some(273),
        name: "Flame Orb",
    },
    &ItemMetadataPastGen {
        id: 58,
        modern_id: Some(281),
        name: "Black Sludge",
    },
    &ItemMetadataPastGen {
        id: 59,
        modern_id: None,
        name: "Max Powder",
    },
    &ItemMetadataPastGen {
        id: 60,
        modern_id: Some(13),
        name: "Dusk Ball",
    },
    &ItemMetadataPastGen {
        id: 61,
        modern_id: Some(14),
        name: "Heal Ball",
    },
    &ItemMetadataPastGen {
        id: 62,
        modern_id: Some(15),
        name: "Quick Ball",
    },
    &ItemMetadataPastGen {
        id: 63,
        modern_id: Some(45),
        name: "HP Up",
    },
    &ItemMetadataPastGen {
        id: 64,
        modern_id: Some(46),
        name: "Protein",
    },
    &ItemMetadataPastGen {
        id: 65,
        modern_id: Some(47),
        name: "Iron",
    },
    &ItemMetadataPastGen {
        id: 66,
        modern_id: Some(48),
        name: "Carbos",
    },
    &ItemMetadataPastGen {
        id: 67,
        modern_id: Some(49),
        name: "Calcium",
    },
    &ItemMetadataPastGen {
        id: 68,
        modern_id: Some(50),
        name: "Rare Candy",
    },
    &ItemMetadataPastGen {
        id: 69,
        modern_id: Some(51),
        name: "PP Up",
    },
    &ItemMetadataPastGen {
        id: 70,
        modern_id: Some(52),
        name: "Zinc",
    },
    &ItemMetadataPastGen {
        id: 71,
        modern_id: Some(53),
        name: "PP Max",
    },
    &ItemMetadataPastGen {
        id: 72,
        modern_id: Some(94),
        name: "Honey",
    },
    &ItemMetadataPastGen {
        id: 73,
        modern_id: None,
        name: "Guard Spec",
    },
    &ItemMetadataPastGen {
        id: 74,
        modern_id: Some(56),
        name: "Dire Hit",
    },
    &ItemMetadataPastGen {
        id: 75,
        modern_id: Some(57),
        name: "X Attack",
    },
    &ItemMetadataPastGen {
        id: 76,
        modern_id: Some(58),
        name: "X Defend",
    },
    &ItemMetadataPastGen {
        id: 77,
        modern_id: Some(59),
        name: "X Speed",
    },
    &ItemMetadataPastGen {
        id: 78,
        modern_id: Some(60),
        name: "X Accuracy",
    },
    &ItemMetadataPastGen {
        id: 79,
        modern_id: None,
        name: "X Special",
    },
    &ItemMetadataPastGen {
        id: 80,
        modern_id: Some(63),
        name: "Poke Doll",
    },
    &ItemMetadataPastGen {
        id: 81,
        modern_id: Some(64),
        name: "Fluffy Tail",
    },
    &ItemMetadataPastGen {
        id: 82,
        modern_id: Some(62),
        name: "X Sp. Def",
    },
    &ItemMetadataPastGen {
        id: 83,
        modern_id: Some(76),
        name: "Super Repel",
    },
    &ItemMetadataPastGen {
        id: 84,
        modern_id: Some(77),
        name: "Max Repel",
    },
    &ItemMetadataPastGen {
        id: 85,
        modern_id: Some(78),
        name: "Escape Rope",
    },
    &ItemMetadataPastGen {
        id: 86,
        modern_id: Some(79),
        name: "Repel",
    },
    &ItemMetadataPastGen {
        id: 87,
        modern_id: None,
        name: "",
    },
    &ItemMetadataPastGen {
        id: 88,
        modern_id: Some(1252),
        name: "Wishing Piece",
    },
    &ItemMetadataPastGen {
        id: 89,
        modern_id: None,
        name: "Dream Mist",
    },
    &ItemMetadataPastGen {
        id: 90,
        modern_id: Some(269),
        name: "Light Clay",
    },
    &ItemMetadataPastGen {
        id: 91,
        modern_id: Some(1611),
        name: "Link Stone",
    },
    &ItemMetadataPastGen {
        id: 92,
        modern_id: Some(458),
        name: "Magma Stone",
    },
    &ItemMetadataPastGen {
        id: 93,
        modern_id: Some(80),
        name: "Sun Stone",
    },
    &ItemMetadataPastGen {
        id: 94,
        modern_id: Some(81),
        name: "Moon Stone",
    },
    &ItemMetadataPastGen {
        id: 95,
        modern_id: Some(82),
        name: "Fire Stone",
    },
    &ItemMetadataPastGen {
        id: 96,
        modern_id: Some(83),
        name: "Thunder Stone",
    },
    &ItemMetadataPastGen {
        id: 97,
        modern_id: Some(84),
        name: "Water Stone",
    },
    &ItemMetadataPastGen {
        id: 98,
        modern_id: Some(85),
        name: "Leaf Stone",
    },
    &ItemMetadataPastGen {
        id: 99,
        modern_id: Some(108),
        name: "Dusk Stone",
    },
    &ItemMetadataPastGen {
        id: 100,
        modern_id: Some(109),
        name: "Dawn Stone",
    },
    &ItemMetadataPastGen {
        id: 101,
        modern_id: Some(107),
        name: "Shiny Stone",
    },
    &ItemMetadataPastGen {
        id: 102,
        modern_id: Some(110),
        name: "Oval Stone",
    },
    &ItemMetadataPastGen {
        id: 103,
        modern_id: Some(86),
        name: "Tiny Mushroom",
    },
    &ItemMetadataPastGen {
        id: 104,
        modern_id: Some(87),
        name: "Big Mushroom",
    },
    &ItemMetadataPastGen {
        id: 105,
        modern_id: Some(849),
        name: "Ice Stone",
    },
    &ItemMetadataPastGen {
        id: 106,
        modern_id: Some(88),
        name: "Pearl",
    },
    &ItemMetadataPastGen {
        id: 107,
        modern_id: Some(89),
        name: "Big Pearl",
    },
    &ItemMetadataPastGen {
        id: 108,
        modern_id: Some(90),
        name: "Stardust",
    },
    &ItemMetadataPastGen {
        id: 109,
        modern_id: Some(91),
        name: "Star Piece",
    },
    &ItemMetadataPastGen {
        id: 110,
        modern_id: Some(92),
        name: "Nugget",
    },
    &ItemMetadataPastGen {
        id: 111,
        modern_id: Some(93),
        name: "Heart Scale",
    },
    &ItemMetadataPastGen {
        id: 112,
        modern_id: Some(321),
        name: "Protector",
    },
    &ItemMetadataPastGen {
        id: 113,
        modern_id: Some(323),
        name: "Magmarizer",
    },
    &ItemMetadataPastGen {
        id: 114,
        modern_id: Some(106),
        name: "Rare Bone",
    },
    &ItemMetadataPastGen {
        id: 115,
        modern_id: Some(537),
        name: "Prism Scale",
    },
    &ItemMetadataPastGen {
        id: 116,
        modern_id: Some(647),
        name: "Sachet",
    },
    &ItemMetadataPastGen {
        id: 117,
        modern_id: Some(646),
        name: "Whipped Dream",
    },
    &ItemMetadataPastGen {
        id: 118,
        modern_id: Some(208),
        name: "Enigma Berry",
    },
    &ItemMetadataPastGen {
        id: 119,
        modern_id: Some(766),
        name: "Mega Cuff",
    },
    &ItemMetadataPastGen {
        id: 120,
        modern_id: Some(749),
        name: "Mega Tiara",
    },
    &ItemMetadataPastGen {
        id: 121,
        modern_id: None,
        name: "Orange Mail",
    },
    &ItemMetadataPastGen {
        id: 122,
        modern_id: None,
        name: "Harbor Mail",
    },
    &ItemMetadataPastGen {
        id: 123,
        modern_id: None,
        name: "Glitter Mail",
    },
    &ItemMetadataPastGen {
        id: 124,
        modern_id: None,
        name: "Mech Mail",
    },
    &ItemMetadataPastGen {
        id: 125,
        modern_id: None,
        name: "Wood Mail",
    },
    &ItemMetadataPastGen {
        id: 126,
        modern_id: None,
        name: "Wave Mail",
    },
    &ItemMetadataPastGen {
        id: 127,
        modern_id: None,
        name: "Bead Mail",
    },
    &ItemMetadataPastGen {
        id: 128,
        modern_id: None,
        name: "Shadow Mail",
    },
    &ItemMetadataPastGen {
        id: 129,
        modern_id: None,
        name: "Tropic Mail",
    },
    &ItemMetadataPastGen {
        id: 130,
        modern_id: None,
        name: "Dream Mail",
    },
    &ItemMetadataPastGen {
        id: 131,
        modern_id: None,
        name: "Fab Mail",
    },
    &ItemMetadataPastGen {
        id: 132,
        modern_id: None,
        name: "Retro Mail",
    },
    &ItemMetadataPastGen {
        id: 133,
        modern_id: Some(149),
        name: "Cheri Berry",
    },
    &ItemMetadataPastGen {
        id: 134,
        modern_id: Some(150),
        name: "Chesto Berry",
    },
    &ItemMetadataPastGen {
        id: 135,
        modern_id: Some(151),
        name: "Pecha Berry",
    },
    &ItemMetadataPastGen {
        id: 136,
        modern_id: Some(152),
        name: "Rawst Berry",
    },
    &ItemMetadataPastGen {
        id: 137,
        modern_id: Some(153),
        name: "Aspear Berry",
    },
    &ItemMetadataPastGen {
        id: 138,
        modern_id: Some(154),
        name: "Leppa Berry",
    },
    &ItemMetadataPastGen {
        id: 139,
        modern_id: Some(155),
        name: "Oran Berry",
    },
    &ItemMetadataPastGen {
        id: 140,
        modern_id: Some(156),
        name: "Persim Berry",
    },
    &ItemMetadataPastGen {
        id: 141,
        modern_id: Some(157),
        name: "Lum Berry",
    },
    &ItemMetadataPastGen {
        id: 142,
        modern_id: Some(158),
        name: "Sitrus Berry",
    },
    &ItemMetadataPastGen {
        id: 143,
        modern_id: Some(159),
        name: "Figy Berry",
    },
    &ItemMetadataPastGen {
        id: 144,
        modern_id: Some(160),
        name: "Wiki Berry",
    },
    &ItemMetadataPastGen {
        id: 145,
        modern_id: Some(161),
        name: "Mago Berry",
    },
    &ItemMetadataPastGen {
        id: 146,
        modern_id: Some(162),
        name: "Aguav Berry",
    },
    &ItemMetadataPastGen {
        id: 147,
        modern_id: Some(163),
        name: "Iapapa Berry",
    },
    &ItemMetadataPastGen {
        id: 148,
        modern_id: Some(164),
        name: "Razz Berry",
    },
    &ItemMetadataPastGen {
        id: 149,
        modern_id: Some(165),
        name: "Bluk Berry",
    },
    &ItemMetadataPastGen {
        id: 150,
        modern_id: Some(166),
        name: "Nanab Berry",
    },
    &ItemMetadataPastGen {
        id: 151,
        modern_id: Some(167),
        name: "Wepear Berry",
    },
    &ItemMetadataPastGen {
        id: 152,
        modern_id: Some(168),
        name: "Pinap Berry",
    },
    &ItemMetadataPastGen {
        id: 153,
        modern_id: Some(169),
        name: "Pomeg Berry",
    },
    &ItemMetadataPastGen {
        id: 154,
        modern_id: Some(170),
        name: "Kelpsy Berry",
    },
    &ItemMetadataPastGen {
        id: 155,
        modern_id: Some(171),
        name: "Qualot Berry",
    },
    &ItemMetadataPastGen {
        id: 156,
        modern_id: Some(172),
        name: "Hondew Berry",
    },
    &ItemMetadataPastGen {
        id: 157,
        modern_id: Some(173),
        name: "Grepa Berry",
    },
    &ItemMetadataPastGen {
        id: 158,
        modern_id: Some(174),
        name: "Tamato Berry",
    },
    &ItemMetadataPastGen {
        id: 159,
        modern_id: Some(175),
        name: "Cornn Berry",
    },
    &ItemMetadataPastGen {
        id: 160,
        modern_id: Some(176),
        name: "Magost Berry",
    },
    &ItemMetadataPastGen {
        id: 161,
        modern_id: Some(177),
        name: "Rabuta Berry",
    },
    &ItemMetadataPastGen {
        id: 162,
        modern_id: Some(178),
        name: "Nomel Berry",
    },
    &ItemMetadataPastGen {
        id: 163,
        modern_id: Some(179),
        name: "Spelon Berry",
    },
    &ItemMetadataPastGen {
        id: 164,
        modern_id: Some(180),
        name: "Pamtre Berry",
    },
    &ItemMetadataPastGen {
        id: 165,
        modern_id: Some(181),
        name: "Watmel Berry",
    },
    &ItemMetadataPastGen {
        id: 166,
        modern_id: Some(182),
        name: "Durin Berry",
    },
    &ItemMetadataPastGen {
        id: 167,
        modern_id: Some(183),
        name: "Belue Berry",
    },
    &ItemMetadataPastGen {
        id: 168,
        modern_id: Some(201),
        name: "Liechi Berry",
    },
    &ItemMetadataPastGen {
        id: 169,
        modern_id: Some(202),
        name: "Ganlon Berry",
    },
    &ItemMetadataPastGen {
        id: 170,
        modern_id: Some(203),
        name: "Salac Berry",
    },
    &ItemMetadataPastGen {
        id: 171,
        modern_id: Some(204),
        name: "Petaya Berry",
    },
    &ItemMetadataPastGen {
        id: 172,
        modern_id: Some(205),
        name: "Apicot Berry",
    },
    &ItemMetadataPastGen {
        id: 173,
        modern_id: Some(206),
        name: "Lansat Berry",
    },
    &ItemMetadataPastGen {
        id: 174,
        modern_id: None,
        name: "Starf Berry (Stat+)",
    },
    &ItemMetadataPastGen {
        id: 175,
        modern_id: None,
        name: "",
    },
    &ItemMetadataPastGen {
        id: 176,
        modern_id: Some(220),
        name: "Choice Band",
    },
    &ItemMetadataPastGen {
        id: 177,
        modern_id: Some(297),
        name: "Choice Specs",
    },
    &ItemMetadataPastGen {
        id: 178,
        modern_id: Some(287),
        name: "Choice Scarf",
    },
    &ItemMetadataPastGen {
        id: 179,
        modern_id: Some(213),
        name: "Bright Powder",
    },
    &ItemMetadataPastGen {
        id: 180,
        modern_id: Some(214),
        name: "White Herb",
    },
    &ItemMetadataPastGen {
        id: 181,
        modern_id: Some(215),
        name: "Macho Brace",
    },
    &ItemMetadataPastGen {
        id: 182,
        modern_id: Some(216),
        name: "Exp Share",
    },
    &ItemMetadataPastGen {
        id: 183,
        modern_id: Some(217),
        name: "Quick Claw",
    },
    &ItemMetadataPastGen {
        id: 184,
        modern_id: Some(218),
        name: "Soothe Bell",
    },
    &ItemMetadataPastGen {
        id: 185,
        modern_id: Some(219),
        name: "Mental Herb",
    },
    &ItemMetadataPastGen {
        id: 186,
        modern_id: None,
        name: "",
    },
    &ItemMetadataPastGen {
        id: 187,
        modern_id: Some(221),
        name: "King’s Rock",
    },
    &ItemMetadataPastGen {
        id: 188,
        modern_id: Some(222),
        name: "Silver Powder",
    },
    &ItemMetadataPastGen {
        id: 189,
        modern_id: Some(223),
        name: "Amulet Coin",
    },
    &ItemMetadataPastGen {
        id: 190,
        modern_id: Some(224),
        name: "Cleanse Tag",
    },
    &ItemMetadataPastGen {
        id: 191,
        modern_id: Some(225),
        name: "Soul Dew",
    },
    &ItemMetadataPastGen {
        id: 192,
        modern_id: Some(226),
        name: "Deep Sea Tooth",
    },
    &ItemMetadataPastGen {
        id: 193,
        modern_id: Some(227),
        name: "Deep Sea Scale",
    },
    &ItemMetadataPastGen {
        id: 194,
        modern_id: Some(228),
        name: "Smoke Ball",
    },
    &ItemMetadataPastGen {
        id: 195,
        modern_id: Some(229),
        name: "Everstone",
    },
    &ItemMetadataPastGen {
        id: 196,
        modern_id: Some(230),
        name: "Focus Band",
    },
    &ItemMetadataPastGen {
        id: 197,
        modern_id: Some(231),
        name: "Lucky Egg",
    },
    &ItemMetadataPastGen {
        id: 198,
        modern_id: Some(232),
        name: "Scope Lens",
    },
    &ItemMetadataPastGen {
        id: 199,
        modern_id: Some(233),
        name: "Metal Coat",
    },
    &ItemMetadataPastGen {
        id: 200,
        modern_id: Some(234),
        name: "Leftovers",
    },
    &ItemMetadataPastGen {
        id: 201,
        modern_id: Some(235),
        name: "Dragon Scale",
    },
    &ItemMetadataPastGen {
        id: 202,
        modern_id: Some(236),
        name: "Light Ball",
    },
    &ItemMetadataPastGen {
        id: 203,
        modern_id: Some(237),
        name: "Soft Sand",
    },
    &ItemMetadataPastGen {
        id: 204,
        modern_id: Some(238),
        name: "Hard Stone",
    },
    &ItemMetadataPastGen {
        id: 205,
        modern_id: Some(239),
        name: "Miracle Seed",
    },
    &ItemMetadataPastGen {
        id: 206,
        modern_id: Some(240),
        name: "Black Glasses",
    },
    &ItemMetadataPastGen {
        id: 207,
        modern_id: Some(241),
        name: "Black Belt",
    },
    &ItemMetadataPastGen {
        id: 208,
        modern_id: Some(242),
        name: "Magnet",
    },
    &ItemMetadataPastGen {
        id: 209,
        modern_id: Some(243),
        name: "Mystic Water",
    },
    &ItemMetadataPastGen {
        id: 210,
        modern_id: Some(244),
        name: "Sharp Beak",
    },
    &ItemMetadataPastGen {
        id: 211,
        modern_id: Some(245),
        name: "Poison Barb",
    },
    &ItemMetadataPastGen {
        id: 212,
        modern_id: Some(246),
        name: "Nevermelt Ice",
    },
    &ItemMetadataPastGen {
        id: 213,
        modern_id: Some(247),
        name: "Spell Tag",
    },
    &ItemMetadataPastGen {
        id: 214,
        modern_id: Some(248),
        name: "Twisted Spoon",
    },
    &ItemMetadataPastGen {
        id: 215,
        modern_id: Some(249),
        name: "Charcoal",
    },
    &ItemMetadataPastGen {
        id: 216,
        modern_id: Some(250),
        name: "Dragon Fang",
    },
    &ItemMetadataPastGen {
        id: 217,
        modern_id: Some(251),
        name: "Silk Scarf",
    },
    &ItemMetadataPastGen {
        id: 218,
        modern_id: Some(252),
        name: "Upgrade",
    },
    &ItemMetadataPastGen {
        id: 219,
        modern_id: Some(253),
        name: "Shell Bell",
    },
    &ItemMetadataPastGen {
        id: 220,
        modern_id: Some(254),
        name: "Sea Incense",
    },
    &ItemMetadataPastGen {
        id: 221,
        modern_id: Some(255),
        name: "Lax Incense",
    },
    &ItemMetadataPastGen {
        id: 222,
        modern_id: Some(256),
        name: "Lucky Punch",
    },
    &ItemMetadataPastGen {
        id: 223,
        modern_id: Some(257),
        name: "Metal Powder",
    },
    &ItemMetadataPastGen {
        id: 224,
        modern_id: Some(258),
        name: "Thick Club",
    },
    &ItemMetadataPastGen {
        id: 225,
        modern_id: Some(259),
        name: "Leek",
    },
    &ItemMetadataPastGen {
        id: 226,
        modern_id: Some(540),
        name: "Rocky Helmet",
    },
    &ItemMetadataPastGen {
        id: 227,
        modern_id: Some(639),
        name: "Weakness Policy",
    },
    &ItemMetadataPastGen {
        id: 228,
        modern_id: Some(582),
        name: "Pearl String",
    },
    &ItemMetadataPastGen {
        id: 229,
        modern_id: Some(581),
        name: "Big Nugget",
    },
    &ItemMetadataPastGen {
        id: 230,
        modern_id: Some(326),
        name: "Razor Claw",
    },
    &ItemMetadataPastGen {
        id: 231,
        modern_id: Some(327),
        name: "Razor Fang",
    },
    &ItemMetadataPastGen {
        id: 232,
        modern_id: Some(325),
        name: "Reaper Cloth",
    },
    &ItemMetadataPastGen {
        id: 233,
        modern_id: Some(324),
        name: "Dubious Disc",
    },
    &ItemMetadataPastGen {
        id: 234,
        modern_id: Some(322),
        name: "Electirizer",
    },
    &ItemMetadataPastGen {
        id: 235,
        modern_id: Some(268),
        name: "Expert Belt",
    },
    &ItemMetadataPastGen {
        id: 236,
        modern_id: Some(271),
        name: "Power Herb",
    },
    &ItemMetadataPastGen {
        id: 237,
        modern_id: Some(265),
        name: "Wide Lens",
    },
    &ItemMetadataPastGen {
        id: 238,
        modern_id: Some(276),
        name: "Zoom Lens",
    },
    &ItemMetadataPastGen {
        id: 239,
        modern_id: Some(280),
        name: "Destiny Knot",
    },
    &ItemMetadataPastGen {
        id: 240,
        modern_id: Some(283),
        name: "Smooth Rock",
    },
    &ItemMetadataPastGen {
        id: 241,
        modern_id: Some(285),
        name: "Damp Rock",
    },
    &ItemMetadataPastGen {
        id: 242,
        modern_id: Some(284),
        name: "Heat Rock",
    },
    &ItemMetadataPastGen {
        id: 243,
        modern_id: Some(282),
        name: "Icy Rock",
    },
    &ItemMetadataPastGen {
        id: 244,
        modern_id: Some(289),
        name: "Power Bracer",
    },
    &ItemMetadataPastGen {
        id: 245,
        modern_id: Some(290),
        name: "Power Belt",
    },
    &ItemMetadataPastGen {
        id: 246,
        modern_id: Some(291),
        name: "Power Lens",
    },
    &ItemMetadataPastGen {
        id: 247,
        modern_id: Some(292),
        name: "Power Band",
    },
    &ItemMetadataPastGen {
        id: 248,
        modern_id: Some(293),
        name: "Power Anklet",
    },
    &ItemMetadataPastGen {
        id: 249,
        modern_id: Some(294),
        name: "Power Weight",
    },
    &ItemMetadataPastGen {
        id: 250,
        modern_id: Some(296),
        name: "Big Root",
    },
    &ItemMetadataPastGen {
        id: 251,
        modern_id: Some(111),
        name: "Odd Keystone",
    },
    &ItemMetadataPastGen {
        id: 252,
        modern_id: Some(319),
        name: "Luck Incense",
    },
    &ItemMetadataPastGen {
        id: 253,
        modern_id: Some(316),
        name: "Full Incense",
    },
    &ItemMetadataPastGen {
        id: 254,
        modern_id: Some(314),
        name: "Odd Incence",
    },
    &ItemMetadataPastGen {
        id: 255,
        modern_id: Some(320),
        name: "Pure Incense",
    },
    &ItemMetadataPastGen {
        id: 256,
        modern_id: Some(315),
        name: "Rock Incense",
    },
    &ItemMetadataPastGen {
        id: 257,
        modern_id: Some(318),
        name: "Rose Incense",
    },
    &ItemMetadataPastGen {
        id: 258,
        modern_id: Some(317),
        name: "Wave Incense",
    },
    &ItemMetadataPastGen {
        id: 259,
        modern_id: Some(718),
        name: "Mach Bike",
    },
    &ItemMetadataPastGen {
        id: 260,
        modern_id: Some(444),
        name: "Coin Case",
    },
    &ItemMetadataPastGen {
        id: 261,
        modern_id: Some(471),
        name: "Dowsing Machine",
    },
    &ItemMetadataPastGen {
        id: 262,
        modern_id: Some(445),
        name: "Old Rod",
    },
    &ItemMetadataPastGen {
        id: 263,
        modern_id: Some(446),
        name: "Good Rod",
    },
    &ItemMetadataPastGen {
        id: 264,
        modern_id: Some(447),
        name: "Super Rod",
    },
    &ItemMetadataPastGen {
        id: 265,
        modern_id: Some(456),
        name: "S.S. Ticket",
    },
    &ItemMetadataPastGen {
        id: 266,
        modern_id: Some(634),
        name: "Grubby Hanky",
    },
    &ItemMetadataPastGen {
        id: 267,
        modern_id: None,
        name: "Uniform",
    },
    &ItemMetadataPastGen {
        id: 268,
        modern_id: Some(1077),
        name: "Dynamax Band",
    },
    &ItemMetadataPastGen {
        id: 269,
        modern_id: None,
        name: "Braille Converter",
    },
    &ItemMetadataPastGen {
        id: 270,
        modern_id: Some(435),
        name: "Fashion Case",
    },
    &ItemMetadataPastGen {
        id: 271,
        modern_id: Some(476),
        name: "Basement Key",
    },
    &ItemMetadataPastGen {
        id: 272,
        modern_id: None,
        name: "Frontier Card",
    },
    &ItemMetadataPastGen {
        id: 273,
        modern_id: None,
        name: "Pokeblock Case",
    },
    &ItemMetadataPastGen {
        id: 274,
        modern_id: Some(725),
        name: "Letter",
    },
    &ItemMetadataPastGen {
        id: 275,
        modern_id: Some(726),
        name: "Eon Ticket",
    },
    &ItemMetadataPastGen {
        id: 276,
        modern_id: Some(534),
        name: "Red Orb",
    },
    &ItemMetadataPastGen {
        id: 277,
        modern_id: Some(535),
        name: "Blue Orb",
    },
    &ItemMetadataPastGen {
        id: 278,
        modern_id: None,
        name: "Stat Scanner",
    },
    &ItemMetadataPastGen {
        id: 279,
        modern_id: Some(728),
        name: "Go-Goggles",
    },
    &ItemMetadataPastGen {
        id: 280,
        modern_id: Some(729),
        name: "Meteorite",
    },
    &ItemMetadataPastGen {
        id: 281,
        modern_id: None,
        name: "Barrier Key",
    },
    &ItemMetadataPastGen {
        id: 282,
        modern_id: None,
        name: "Key Card: 2",
    },
    &ItemMetadataPastGen {
        id: 283,
        modern_id: None,
        name: "Key Card: 3",
    },
    &ItemMetadataPastGen {
        id: 284,
        modern_id: Some(733),
        name: "Rm. 6 Key",
    },
    &ItemMetadataPastGen {
        id: 285,
        modern_id: None,
        name: "Storage Key (Abandoned Ship)",
    },
    &ItemMetadataPastGen {
        id: 286,
        modern_id: Some(99),
        name: "Root Fossil",
    },
    &ItemMetadataPastGen {
        id: 287,
        modern_id: Some(100),
        name: "Claw Fossil",
    },
    &ItemMetadataPastGen {
        id: 288,
        modern_id: Some(735),
        name: "Devon Scope",
    },
    &ItemMetadataPastGen {
        id: 289,
        modern_id: Some(328),
        name: "TM01",
    },
    &ItemMetadataPastGen {
        id: 290,
        modern_id: Some(329),
        name: "TM02",
    },
    &ItemMetadataPastGen {
        id: 291,
        modern_id: Some(330),
        name: "TM03",
    },
    &ItemMetadataPastGen {
        id: 292,
        modern_id: Some(331),
        name: "TM04",
    },
    &ItemMetadataPastGen {
        id: 293,
        modern_id: Some(332),
        name: "TM05",
    },
    &ItemMetadataPastGen {
        id: 294,
        modern_id: Some(333),
        name: "TM06",
    },
    &ItemMetadataPastGen {
        id: 295,
        modern_id: Some(334),
        name: "TM07",
    },
    &ItemMetadataPastGen {
        id: 296,
        modern_id: Some(335),
        name: "TM08",
    },
    &ItemMetadataPastGen {
        id: 297,
        modern_id: Some(336),
        name: "TM09",
    },
    &ItemMetadataPastGen {
        id: 298,
        modern_id: Some(337),
        name: "TM10",
    },
    &ItemMetadataPastGen {
        id: 299,
        modern_id: Some(338),
        name: "TM11",
    },
    &ItemMetadataPastGen {
        id: 300,
        modern_id: Some(339),
        name: "TM12",
    },
    &ItemMetadataPastGen {
        id: 301,
        modern_id: Some(340),
        name: "TM13",
    },
    &ItemMetadataPastGen {
        id: 302,
        modern_id: Some(341),
        name: "TM14",
    },
    &ItemMetadataPastGen {
        id: 303,
        modern_id: Some(342),
        name: "TM15",
    },
    &ItemMetadataPastGen {
        id: 304,
        modern_id: Some(343),
        name: "TM16",
    },
    &ItemMetadataPastGen {
        id: 305,
        modern_id: Some(344),
        name: "TM17",
    },
    &ItemMetadataPastGen {
        id: 306,
        modern_id: Some(345),
        name: "TM18",
    },
    &ItemMetadataPastGen {
        id: 307,
        modern_id: Some(346),
        name: "TM19",
    },
    &ItemMetadataPastGen {
        id: 308,
        modern_id: Some(347),
        name: "TM20",
    },
    &ItemMetadataPastGen {
        id: 309,
        modern_id: Some(348),
        name: "TM21",
    },
    &ItemMetadataPastGen {
        id: 310,
        modern_id: Some(349),
        name: "TM22",
    },
    &ItemMetadataPastGen {
        id: 311,
        modern_id: Some(350),
        name: "TM23",
    },
    &ItemMetadataPastGen {
        id: 312,
        modern_id: Some(351),
        name: "TM24",
    },
    &ItemMetadataPastGen {
        id: 313,
        modern_id: Some(352),
        name: "TM25",
    },
    &ItemMetadataPastGen {
        id: 314,
        modern_id: Some(353),
        name: "TM26",
    },
    &ItemMetadataPastGen {
        id: 315,
        modern_id: Some(354),
        name: "TM27",
    },
    &ItemMetadataPastGen {
        id: 316,
        modern_id: Some(355),
        name: "TM28",
    },
    &ItemMetadataPastGen {
        id: 317,
        modern_id: Some(356),
        name: "TM29",
    },
    &ItemMetadataPastGen {
        id: 318,
        modern_id: Some(357),
        name: "TM30",
    },
    &ItemMetadataPastGen {
        id: 319,
        modern_id: Some(358),
        name: "TM31",
    },
    &ItemMetadataPastGen {
        id: 320,
        modern_id: Some(359),
        name: "TM32",
    },
    &ItemMetadataPastGen {
        id: 321,
        modern_id: Some(360),
        name: "TM33",
    },
    &ItemMetadataPastGen {
        id: 322,
        modern_id: Some(361),
        name: "TM34",
    },
    &ItemMetadataPastGen {
        id: 323,
        modern_id: Some(362),
        name: "TM35",
    },
    &ItemMetadataPastGen {
        id: 324,
        modern_id: Some(363),
        name: "TM36",
    },
    &ItemMetadataPastGen {
        id: 325,
        modern_id: Some(364),
        name: "TM37",
    },
    &ItemMetadataPastGen {
        id: 326,
        modern_id: Some(365),
        name: "TM38",
    },
    &ItemMetadataPastGen {
        id: 327,
        modern_id: Some(366),
        name: "TM39",
    },
    &ItemMetadataPastGen {
        id: 328,
        modern_id: Some(367),
        name: "TM40",
    },
    &ItemMetadataPastGen {
        id: 329,
        modern_id: Some(368),
        name: "TM41",
    },
    &ItemMetadataPastGen {
        id: 330,
        modern_id: Some(369),
        name: "TM42",
    },
    &ItemMetadataPastGen {
        id: 331,
        modern_id: Some(370),
        name: "TM43",
    },
    &ItemMetadataPastGen {
        id: 332,
        modern_id: Some(371),
        name: "TM44",
    },
    &ItemMetadataPastGen {
        id: 333,
        modern_id: Some(372),
        name: "TM45",
    },
    &ItemMetadataPastGen {
        id: 334,
        modern_id: Some(373),
        name: "TM46",
    },
    &ItemMetadataPastGen {
        id: 335,
        modern_id: Some(374),
        name: "TM47",
    },
    &ItemMetadataPastGen {
        id: 336,
        modern_id: Some(375),
        name: "TM48",
    },
    &ItemMetadataPastGen {
        id: 337,
        modern_id: Some(376),
        name: "TM49",
    },
    &ItemMetadataPastGen {
        id: 338,
        modern_id: Some(377),
        name: "TM50",
    },
    &ItemMetadataPastGen {
        id: 339,
        modern_id: Some(378),
        name: "TM51",
    },
    &ItemMetadataPastGen {
        id: 340,
        modern_id: Some(379),
        name: "TM52",
    },
    &ItemMetadataPastGen {
        id: 341,
        modern_id: Some(380),
        name: "TM53",
    },
    &ItemMetadataPastGen {
        id: 342,
        modern_id: Some(381),
        name: "TM54",
    },
    &ItemMetadataPastGen {
        id: 343,
        modern_id: Some(382),
        name: "TM55",
    },
    &ItemMetadataPastGen {
        id: 344,
        modern_id: Some(383),
        name: "TM56",
    },
    &ItemMetadataPastGen {
        id: 345,
        modern_id: Some(384),
        name: "TM57",
    },
    &ItemMetadataPastGen {
        id: 346,
        modern_id: Some(385),
        name: "TM58",
    },
    &ItemMetadataPastGen {
        id: 347,
        modern_id: None,
        name: "Dead Value",
    },
    &ItemMetadataPastGen {
        id: 348,
        modern_id: None,
        name: "Mission Log",
    },
    &ItemMetadataPastGen {
        id: 349,
        modern_id: Some(459),
        name: "Parcel",
    },
    &ItemMetadataPastGen {
        id: 350,
        modern_id: Some(532),
        name: "Jade Orb",
    },
    &ItemMetadataPastGen {
        id: 351,
        modern_id: None,
        name: "Grimy Socks",
    },
    &ItemMetadataPastGen {
        id: 352,
        modern_id: None,
        name: "Package",
    },
    &ItemMetadataPastGen {
        id: 353,
        modern_id: Some(696),
        name: "Mega Ring",
    },
    &ItemMetadataPastGen {
        id: 354,
        modern_id: Some(103),
        name: "Old Amber",
    },
    &ItemMetadataPastGen {
        id: 355,
        modern_id: None,
        name: "Key Card: 1",
    },
    &ItemMetadataPastGen {
        id: 356,
        modern_id: None,
        name: "Time Turner",
    },
    &ItemMetadataPastGen {
        id: 357,
        modern_id: Some(101),
        name: "Helix Fossil",
    },
    &ItemMetadataPastGen {
        id: 358,
        modern_id: Some(102),
        name: "Dome Fossil",
    },
    &ItemMetadataPastGen {
        id: 359,
        modern_id: None,
        name: "Baby Monitor",
    },
    &ItemMetadataPastGen {
        id: 360,
        modern_id: None,
        name: "Motorcycle",
    },
    &ItemMetadataPastGen {
        id: 361,
        modern_id: Some(442),
        name: "Town Map",
    },
    &ItemMetadataPastGen {
        id: 362,
        modern_id: None,
        name: "Vs Seeker",
    },
    &ItemMetadataPastGen {
        id: 363,
        modern_id: None,
        name: "Oddish Leaves",
    },
    &ItemMetadataPastGen {
        id: 364,
        modern_id: None,
        name: "Tm Case",
    },
    &ItemMetadataPastGen {
        id: 365,
        modern_id: None,
        name: "Berry Pouch",
    },
    &ItemMetadataPastGen {
        id: 366,
        modern_id: None,
        name: "Adm",
    },
    &ItemMetadataPastGen {
        id: 367,
        modern_id: None,
        name: "Tri-Pass",
    },
    &ItemMetadataPastGen {
        id: 368,
        modern_id: None,
        name: "Porta-Pc",
    },
    &ItemMetadataPastGen {
        id: 369,
        modern_id: None,
        name: "Trainer Catalogue",
    },
    &ItemMetadataPastGen {
        id: 370,
        modern_id: None,
        name: "Glimmer Pass",
    },
    &ItemMetadataPastGen {
        id: 371,
        modern_id: Some(1267),
        name: "Catching Charm",
    },
    &ItemMetadataPastGen {
        id: 372,
        modern_id: None,
        name: "Powder “Berry” Jar",
    },
    &ItemMetadataPastGen {
        id: 373,
        modern_id: Some(453),
        name: "Lunar Wing",
    },
    &ItemMetadataPastGen {
        id: 374,
        modern_id: Some(454),
        name: "Member Card",
    },
    &ItemMetadataPastGen {
        id: 375,
        modern_id: Some(386),
        name: "TM59",
    },
    &ItemMetadataPastGen {
        id: 376,
        modern_id: Some(387),
        name: "TM60",
    },
    &ItemMetadataPastGen {
        id: 377,
        modern_id: Some(388),
        name: "TM61",
    },
    &ItemMetadataPastGen {
        id: 378,
        modern_id: Some(389),
        name: "TM62",
    },
    &ItemMetadataPastGen {
        id: 379,
        modern_id: Some(390),
        name: "TM63",
    },
    &ItemMetadataPastGen {
        id: 380,
        modern_id: Some(391),
        name: "TM64",
    },
    &ItemMetadataPastGen {
        id: 381,
        modern_id: Some(392),
        name: "TM65",
    },
    &ItemMetadataPastGen {
        id: 382,
        modern_id: Some(393),
        name: "TM66",
    },
    &ItemMetadataPastGen {
        id: 383,
        modern_id: Some(394),
        name: "TM67",
    },
    &ItemMetadataPastGen {
        id: 384,
        modern_id: Some(395),
        name: "TM68",
    },
    &ItemMetadataPastGen {
        id: 385,
        modern_id: Some(396),
        name: "TM69",
    },
    &ItemMetadataPastGen {
        id: 386,
        modern_id: Some(397),
        name: "TM70",
    },
    &ItemMetadataPastGen {
        id: 387,
        modern_id: Some(398),
        name: "TM71",
    },
    &ItemMetadataPastGen {
        id: 388,
        modern_id: Some(399),
        name: "TM72",
    },
    &ItemMetadataPastGen {
        id: 389,
        modern_id: Some(400),
        name: "TM73",
    },
    &ItemMetadataPastGen {
        id: 390,
        modern_id: Some(401),
        name: "TM74",
    },
    &ItemMetadataPastGen {
        id: 391,
        modern_id: Some(402),
        name: "TM75",
    },
    &ItemMetadataPastGen {
        id: 392,
        modern_id: Some(403),
        name: "TM76",
    },
    &ItemMetadataPastGen {
        id: 393,
        modern_id: Some(404),
        name: "TM77",
    },
    &ItemMetadataPastGen {
        id: 394,
        modern_id: Some(405),
        name: "TM78",
    },
    &ItemMetadataPastGen {
        id: 395,
        modern_id: Some(406),
        name: "TM79",
    },
    &ItemMetadataPastGen {
        id: 396,
        modern_id: Some(407),
        name: "TM80",
    },
    &ItemMetadataPastGen {
        id: 397,
        modern_id: Some(408),
        name: "TM81",
    },
    &ItemMetadataPastGen {
        id: 398,
        modern_id: Some(409),
        name: "TM82",
    },
    &ItemMetadataPastGen {
        id: 399,
        modern_id: Some(410),
        name: "TM83",
    },
    &ItemMetadataPastGen {
        id: 400,
        modern_id: Some(411),
        name: "TM84",
    },
    &ItemMetadataPastGen {
        id: 401,
        modern_id: Some(412),
        name: "TM85",
    },
    &ItemMetadataPastGen {
        id: 402,
        modern_id: Some(413),
        name: "TM86",
    },
    &ItemMetadataPastGen {
        id: 403,
        modern_id: Some(414),
        name: "TM87",
    },
    &ItemMetadataPastGen {
        id: 404,
        modern_id: Some(415),
        name: "TM88",
    },
    &ItemMetadataPastGen {
        id: 405,
        modern_id: Some(416),
        name: "TM89",
    },
    &ItemMetadataPastGen {
        id: 406,
        modern_id: Some(417),
        name: "TM90",
    },
    &ItemMetadataPastGen {
        id: 407,
        modern_id: Some(418),
        name: "TM91",
    },
    &ItemMetadataPastGen {
        id: 408,
        modern_id: Some(419),
        name: "TM92",
    },
    &ItemMetadataPastGen {
        id: 409,
        modern_id: Some(618),
        name: "TM93",
    },
    &ItemMetadataPastGen {
        id: 410,
        modern_id: Some(619),
        name: "TM94",
    },
    &ItemMetadataPastGen {
        id: 411,
        modern_id: Some(620),
        name: "TM95",
    },
    &ItemMetadataPastGen {
        id: 412,
        modern_id: Some(690),
        name: "TM96",
    },
    &ItemMetadataPastGen {
        id: 413,
        modern_id: Some(691),
        name: "TM97",
    },
    &ItemMetadataPastGen {
        id: 414,
        modern_id: Some(692),
        name: "TM98",
    },
    &ItemMetadataPastGen {
        id: 415,
        modern_id: Some(693),
        name: "TM99",
    },
    &ItemMetadataPastGen {
        id: 416,
        modern_id: Some(694),
        name: "TM100",
    },
    &ItemMetadataPastGen {
        id: 417,
        modern_id: Some(2161),
        name: "TM101",
    },
    &ItemMetadataPastGen {
        id: 418,
        modern_id: Some(2162),
        name: "TM102",
    },
    &ItemMetadataPastGen {
        id: 419,
        modern_id: Some(2163),
        name: "TM103",
    },
    &ItemMetadataPastGen {
        id: 420,
        modern_id: Some(2164),
        name: "TM104",
    },
    &ItemMetadataPastGen {
        id: 421,
        modern_id: Some(2165),
        name: "TM105",
    },
    &ItemMetadataPastGen {
        id: 422,
        modern_id: Some(2166),
        name: "TM106",
    },
    &ItemMetadataPastGen {
        id: 423,
        modern_id: Some(2167),
        name: "TM107",
    },
    &ItemMetadataPastGen {
        id: 424,
        modern_id: Some(2168),
        name: "TM108",
    },
    &ItemMetadataPastGen {
        id: 425,
        modern_id: Some(2169),
        name: "TM109",
    },
    &ItemMetadataPastGen {
        id: 426,
        modern_id: Some(2170),
        name: "TM110",
    },
    &ItemMetadataPastGen {
        id: 427,
        modern_id: Some(2171),
        name: "TM111",
    },
    &ItemMetadataPastGen {
        id: 428,
        modern_id: Some(2172),
        name: "TM112",
    },
    &ItemMetadataPastGen {
        id: 429,
        modern_id: Some(2173),
        name: "TM113",
    },
    &ItemMetadataPastGen {
        id: 430,
        modern_id: Some(2174),
        name: "TM114",
    },
    &ItemMetadataPastGen {
        id: 431,
        modern_id: Some(2175),
        name: "TM115",
    },
    &ItemMetadataPastGen {
        id: 432,
        modern_id: Some(2176),
        name: "TM116",
    },
    &ItemMetadataPastGen {
        id: 433,
        modern_id: Some(2177),
        name: "TM117",
    },
    &ItemMetadataPastGen {
        id: 434,
        modern_id: Some(2178),
        name: "TM118",
    },
    &ItemMetadataPastGen {
        id: 435,
        modern_id: Some(2179),
        name: "TM119",
    },
    &ItemMetadataPastGen {
        id: 436,
        modern_id: Some(2180),
        name: "TM120",
    },
    &ItemMetadataPastGen {
        id: 437,
        modern_id: Some(420),
        name: "HM01",
    },
    &ItemMetadataPastGen {
        id: 438,
        modern_id: Some(421),
        name: "HM02",
    },
    &ItemMetadataPastGen {
        id: 439,
        modern_id: Some(422),
        name: "HM03",
    },
    &ItemMetadataPastGen {
        id: 440,
        modern_id: Some(423),
        name: "HM04",
    },
    &ItemMetadataPastGen {
        id: 441,
        modern_id: Some(424),
        name: "HM05",
    },
    &ItemMetadataPastGen {
        id: 442,
        modern_id: Some(425),
        name: "HM06",
    },
    &ItemMetadataPastGen {
        id: 443,
        modern_id: Some(737),
        name: "HM07",
    },
    &ItemMetadataPastGen {
        id: 444,
        modern_id: None,
        name: "HM08",
    },
    &ItemMetadataPastGen {
        id: 445,
        modern_id: Some(923),
        name: "Necrozium Z",
    },
    &ItemMetadataPastGen {
        id: 446,
        modern_id: Some(659),
        name: "Venusaurite",
    },
    &ItemMetadataPastGen {
        id: 447,
        modern_id: Some(660),
        name: "Charizardite X",
    },
    &ItemMetadataPastGen {
        id: 448,
        modern_id: Some(678),
        name: "Charizardite Y",
    },
    &ItemMetadataPastGen {
        id: 449,
        modern_id: Some(661),
        name: "Blastoisinite",
    },
    &ItemMetadataPastGen {
        id: 450,
        modern_id: Some(770),
        name: "Beedrillite",
    },
    &ItemMetadataPastGen {
        id: 451,
        modern_id: Some(762),
        name: "Pidgeotite",
    },
    &ItemMetadataPastGen {
        id: 452,
        modern_id: Some(679),
        name: "Alakazite",
    },
    &ItemMetadataPastGen {
        id: 453,
        modern_id: Some(760),
        name: "Slowbronite",
    },
    &ItemMetadataPastGen {
        id: 454,
        modern_id: Some(656),
        name: "Gengarite",
    },
    &ItemMetadataPastGen {
        id: 455,
        modern_id: Some(675),
        name: "Kangaskhite",
    },
    &ItemMetadataPastGen {
        id: 456,
        modern_id: Some(671),
        name: "Pinsirite",
    },
    &ItemMetadataPastGen {
        id: 457,
        modern_id: Some(676),
        name: "Gyaradosite",
    },
    &ItemMetadataPastGen {
        id: 458,
        modern_id: Some(672),
        name: "Aerodactylite",
    },
    &ItemMetadataPastGen {
        id: 459,
        modern_id: Some(662),
        name: "Mewtwonite X",
    },
    &ItemMetadataPastGen {
        id: 460,
        modern_id: Some(663),
        name: "Mewtwonite Y",
    },
    &ItemMetadataPastGen {
        id: 461,
        modern_id: Some(658),
        name: "Ampharosite",
    },
    &ItemMetadataPastGen {
        id: 462,
        modern_id: Some(761),
        name: "Steelixite",
    },
    &ItemMetadataPastGen {
        id: 463,
        modern_id: Some(670),
        name: "Scizorite",
    },
    &ItemMetadataPastGen {
        id: 464,
        modern_id: Some(680),
        name: "Heracronite",
    },
    &ItemMetadataPastGen {
        id: 465,
        modern_id: Some(666),
        name: "Houndoominite",
    },
    &ItemMetadataPastGen {
        id: 466,
        modern_id: Some(669),
        name: "Tyranitarite",
    },
    &ItemMetadataPastGen {
        id: 467,
        modern_id: Some(753),
        name: "Sceptilite",
    },
    &ItemMetadataPastGen {
        id: 468,
        modern_id: Some(664),
        name: "Blazikenite",
    },
    &ItemMetadataPastGen {
        id: 469,
        modern_id: Some(752),
        name: "Swampertite",
    },
    &ItemMetadataPastGen {
        id: 470,
        modern_id: Some(657),
        name: "Gardevoirite",
    },
    &ItemMetadataPastGen {
        id: 471,
        modern_id: Some(754),
        name: "Sablenite",
    },
    &ItemMetadataPastGen {
        id: 472,
        modern_id: Some(681),
        name: "Mawilite",
    },
    &ItemMetadataPastGen {
        id: 473,
        modern_id: Some(667),
        name: "Aggronite",
    },
    &ItemMetadataPastGen {
        id: 474,
        modern_id: Some(665),
        name: "Medichamite",
    },
    &ItemMetadataPastGen {
        id: 475,
        modern_id: Some(682),
        name: "Manectite",
    },
    &ItemMetadataPastGen {
        id: 476,
        modern_id: Some(759),
        name: "Sharpedonite",
    },
    &ItemMetadataPastGen {
        id: 477,
        modern_id: Some(767),
        name: "Cameruptite",
    },
    &ItemMetadataPastGen {
        id: 478,
        modern_id: Some(755),
        name: "Altarianite",
    },
    &ItemMetadataPastGen {
        id: 479,
        modern_id: Some(668),
        name: "Banettite",
    },
    &ItemMetadataPastGen {
        id: 480,
        modern_id: Some(677),
        name: "Absolite",
    },
    &ItemMetadataPastGen {
        id: 481,
        modern_id: Some(763),
        name: "Glalitite",
    },
    &ItemMetadataPastGen {
        id: 482,
        modern_id: Some(769),
        name: "Salamencite",
    },
    &ItemMetadataPastGen {
        id: 483,
        modern_id: Some(758),
        name: "Metagrossite",
    },
    &ItemMetadataPastGen {
        id: 484,
        modern_id: Some(684),
        name: "Latiasite",
    },
    &ItemMetadataPastGen {
        id: 485,
        modern_id: Some(685),
        name: "Latiosite",
    },
    &ItemMetadataPastGen {
        id: 486,
        modern_id: Some(768),
        name: "Lopunnite",
    },
    &ItemMetadataPastGen {
        id: 487,
        modern_id: Some(683),
        name: "Garchompite",
    },
    &ItemMetadataPastGen {
        id: 488,
        modern_id: Some(673),
        name: "Lucarionite",
    },
    &ItemMetadataPastGen {
        id: 489,
        modern_id: Some(674),
        name: "Abomasite",
    },
    &ItemMetadataPastGen {
        id: 490,
        modern_id: Some(756),
        name: "Galladite",
    },
    &ItemMetadataPastGen {
        id: 491,
        modern_id: Some(757),
        name: "Audinite",
    },
    &ItemMetadataPastGen {
        id: 492,
        modern_id: Some(764),
        name: "Diancite",
    },
    &ItemMetadataPastGen {
        id: 493,
        modern_id: Some(650),
        name: "Safety Goggles",
    },
    &ItemMetadataPastGen {
        id: 494,
        modern_id: Some(640),
        name: "Assault Vest",
    },
    &ItemMetadataPastGen {
        id: 495,
        modern_id: Some(538),
        name: "Eviolite",
    },
    &ItemMetadataPastGen {
        id: 496,
        modern_id: Some(303),
        name: "Fist Plate",
    },
    &ItemMetadataPastGen {
        id: 497,
        modern_id: Some(306),
        name: "Sky Plate",
    },
    &ItemMetadataPastGen {
        id: 498,
        modern_id: Some(304),
        name: "Toxic Plate",
    },
    &ItemMetadataPastGen {
        id: 499,
        modern_id: Some(305),
        name: "Earth Plate",
    },
    &ItemMetadataPastGen {
        id: 500,
        modern_id: Some(309),
        name: "Stone Plate",
    },
    &ItemMetadataPastGen {
        id: 501,
        modern_id: Some(308),
        name: "Insect Plate",
    },
    &ItemMetadataPastGen {
        id: 502,
        modern_id: Some(310),
        name: "Spooky Plate",
    },
    &ItemMetadataPastGen {
        id: 503,
        modern_id: Some(313),
        name: "Iron Plate",
    },
    &ItemMetadataPastGen {
        id: 504,
        modern_id: Some(298),
        name: "Flame Plate",
    },
    &ItemMetadataPastGen {
        id: 505,
        modern_id: Some(299),
        name: "Splash Plate",
    },
    &ItemMetadataPastGen {
        id: 506,
        modern_id: Some(301),
        name: "Meadow Plate",
    },
    &ItemMetadataPastGen {
        id: 507,
        modern_id: Some(300),
        name: "Zap Plate",
    },
    &ItemMetadataPastGen {
        id: 508,
        modern_id: Some(307),
        name: "Mind Plate",
    },
    &ItemMetadataPastGen {
        id: 509,
        modern_id: Some(302),
        name: "Icicle Plate",
    },
    &ItemMetadataPastGen {
        id: 510,
        modern_id: Some(311),
        name: "Draco Plate",
    },
    &ItemMetadataPastGen {
        id: 511,
        modern_id: Some(312),
        name: "Dread Plate",
    },
    &ItemMetadataPastGen {
        id: 512,
        modern_id: Some(644),
        name: "Pixie Plate",
    },
    &ItemMetadataPastGen {
        id: 513,
        modern_id: Some(583),
        name: "Comet Shard",
    },
    &ItemMetadataPastGen {
        id: 514,
        modern_id: Some(580),
        name: "Balm Mushroom",
    },
    &ItemMetadataPastGen {
        id: 515,
        modern_id: Some(852),
        name: "Big Malasada",
    },
    &ItemMetadataPastGen {
        id: 516,
        modern_id: Some(591),
        name: "Casteliacone",
    },
    &ItemMetadataPastGen {
        id: 517,
        modern_id: Some(708),
        name: "Lumiose Galette",
    },
    &ItemMetadataPastGen {
        id: 518,
        modern_id: None,
        name: "Ragecandybar",
    },
    &ItemMetadataPastGen {
        id: 519,
        modern_id: Some(709),
        name: "Shalour Sable",
    },
    &ItemMetadataPastGen {
        id: 520,
        modern_id: Some(54),
        name: "Old Gateau",
    },
    &ItemMetadataPastGen {
        id: 521,
        modern_id: Some(105),
        name: "Skull Fossil",
    },
    &ItemMetadataPastGen {
        id: 522,
        modern_id: Some(104),
        name: "Armour Fossil",
    },
    &ItemMetadataPastGen {
        id: 523,
        modern_id: Some(572),
        name: "Cover Fossil",
    },
    &ItemMetadataPastGen {
        id: 524,
        modern_id: Some(573),
        name: "Plume Fossil",
    },
    &ItemMetadataPastGen {
        id: 525,
        modern_id: Some(710),
        name: "Jaw Fossil",
    },
    &ItemMetadataPastGen {
        id: 526,
        modern_id: Some(711),
        name: "Sail Fossil",
    },
    &ItemMetadataPastGen {
        id: 527,
        modern_id: None,
        name: "Mega Earring",
    },
    &ItemMetadataPastGen {
        id: 528,
        modern_id: Some(716),
        name: "Mega Charm",
    },
    &ItemMetadataPastGen {
        id: 529,
        modern_id: Some(744),
        name: "Mega Bracelet",
    },
    &ItemMetadataPastGen {
        id: 530,
        modern_id: Some(750),
        name: "Mega Anklet",
    },
    &ItemMetadataPastGen {
        id: 531,
        modern_id: Some(745),
        name: "Mega Pendant",
    },
    &ItemMetadataPastGen {
        id: 532,
        modern_id: Some(135),
        name: "Adamant Orb",
    },
    &ItemMetadataPastGen {
        id: 533,
        modern_id: Some(136),
        name: "Lustrous Orb",
    },
    &ItemMetadataPastGen {
        id: 534,
        modern_id: Some(112),
        name: "Griseous Orb",
    },
    &ItemMetadataPastGen {
        id: 535,
        modern_id: Some(466),
        name: "Gracidea",
    },
    &ItemMetadataPastGen {
        id: 536,
        modern_id: Some(628),
        name: "DNA Splicers",
    },
    &ItemMetadataPastGen {
        id: 537,
        modern_id: Some(638),
        name: "Reveal Glass",
    },
    &ItemMetadataPastGen {
        id: 538,
        modern_id: Some(765),
        name: "Prison Bottle",
    },
    &ItemMetadataPastGen {
        id: 539,
        modern_id: Some(184),
        name: "Occa Berry",
    },
    &ItemMetadataPastGen {
        id: 540,
        modern_id: Some(185),
        name: "Passho Berry",
    },
    &ItemMetadataPastGen {
        id: 541,
        modern_id: Some(186),
        name: "Wacan Berry",
    },
    &ItemMetadataPastGen {
        id: 542,
        modern_id: Some(187),
        name: "Rindo Berry",
    },
    &ItemMetadataPastGen {
        id: 543,
        modern_id: Some(188),
        name: "Yache Berry",
    },
    &ItemMetadataPastGen {
        id: 544,
        modern_id: Some(189),
        name: "Chople Berry",
    },
    &ItemMetadataPastGen {
        id: 545,
        modern_id: Some(190),
        name: "Kebia Berry",
    },
    &ItemMetadataPastGen {
        id: 546,
        modern_id: Some(191),
        name: "Shuca Berry",
    },
    &ItemMetadataPastGen {
        id: 547,
        modern_id: Some(192),
        name: "Coba Berry",
    },
    &ItemMetadataPastGen {
        id: 548,
        modern_id: Some(193),
        name: "Payapa Berry",
    },
    &ItemMetadataPastGen {
        id: 549,
        modern_id: Some(194),
        name: "Tanga Berry",
    },
    &ItemMetadataPastGen {
        id: 550,
        modern_id: Some(195),
        name: "Charti Berry",
    },
    &ItemMetadataPastGen {
        id: 551,
        modern_id: Some(196),
        name: "Kasib Berry",
    },
    &ItemMetadataPastGen {
        id: 552,
        modern_id: Some(197),
        name: "Haban Berry",
    },
    &ItemMetadataPastGen {
        id: 553,
        modern_id: Some(198),
        name: "Colbur Berry",
    },
    &ItemMetadataPastGen {
        id: 554,
        modern_id: Some(199),
        name: "Babiri Berry",
    },
    &ItemMetadataPastGen {
        id: 555,
        modern_id: Some(200),
        name: "Chilan Berry",
    },
    &ItemMetadataPastGen {
        id: 556,
        modern_id: Some(209),
        name: "Micle Berry",
    },
    &ItemMetadataPastGen {
        id: 557,
        modern_id: Some(210),
        name: "Custap Berry",
    },
    &ItemMetadataPastGen {
        id: 558,
        modern_id: Some(211),
        name: "Jaboca Berry",
    },
    &ItemMetadataPastGen {
        id: 559,
        modern_id: Some(212),
        name: "Rowap Berry",
    },
    &ItemMetadataPastGen {
        id: 560,
        modern_id: Some(686),
        name: "Roseli Berry",
    },
    &ItemMetadataPastGen {
        id: 561,
        modern_id: Some(687),
        name: "Kee Berry",
    },
    &ItemMetadataPastGen {
        id: 562,
        modern_id: Some(688),
        name: "Maranga Berry",
    },
    &ItemMetadataPastGen {
        id: 563,
        modern_id: Some(545),
        name: "Absorb Bulb",
    },
    &ItemMetadataPastGen {
        id: 564,
        modern_id: Some(541),
        name: "Air Balloon",
    },
    &ItemMetadataPastGen {
        id: 565,
        modern_id: Some(846),
        name: "Adrenal Orb",
    },
    &ItemMetadataPastGen {
        id: 566,
        modern_id: Some(544),
        name: "Binding Band",
    },
    &ItemMetadataPastGen {
        id: 567,
        modern_id: Some(546),
        name: "Cell Battery",
    },
    &ItemMetadataPastGen {
        id: 568,
        modern_id: Some(547),
        name: "Eject Button",
    },
    &ItemMetadataPastGen {
        id: 569,
        modern_id: Some(539),
        name: "Float Stone",
    },
    &ItemMetadataPastGen {
        id: 570,
        modern_id: Some(275),
        name: "Focus Sash",
    },
    &ItemMetadataPastGen {
        id: 571,
        modern_id: Some(286),
        name: "Grip Claw",
    },
    &ItemMetadataPastGen {
        id: 572,
        modern_id: Some(278),
        name: "Iron Ball",
    },
    &ItemMetadataPastGen {
        id: 573,
        modern_id: Some(279),
        name: "Lagging Tail",
    },
    &ItemMetadataPastGen {
        id: 574,
        modern_id: Some(648),
        name: "Luminous Moss",
    },
    &ItemMetadataPastGen {
        id: 575,
        modern_id: Some(274),
        name: "Quick Powder",
    },
    &ItemMetadataPastGen {
        id: 576,
        modern_id: Some(277),
        name: "Metronome",
    },
    &ItemMetadataPastGen {
        id: 577,
        modern_id: Some(266),
        name: "Muscle Band",
    },
    &ItemMetadataPastGen {
        id: 578,
        modern_id: Some(880),
        name: "Protective Pads",
    },
    &ItemMetadataPastGen {
        id: 579,
        modern_id: Some(542),
        name: "Red Card",
    },
    &ItemMetadataPastGen {
        id: 580,
        modern_id: Some(543),
        name: "Ring Target",
    },
    &ItemMetadataPastGen {
        id: 581,
        modern_id: Some(295),
        name: "Shed Shell",
    },
    &ItemMetadataPastGen {
        id: 582,
        modern_id: Some(649),
        name: "Snowball",
    },
    &ItemMetadataPastGen {
        id: 583,
        modern_id: Some(288),
        name: "Sticky Barb",
    },
    &ItemMetadataPastGen {
        id: 584,
        modern_id: Some(879),
        name: "Terrain Extender",
    },
    &ItemMetadataPastGen {
        id: 585,
        modern_id: Some(267),
        name: "Wise Glasses",
    },
    &ItemMetadataPastGen {
        id: 586,
        modern_id: Some(881),
        name: "Electric Seed",
    },
    &ItemMetadataPastGen {
        id: 587,
        modern_id: Some(884),
        name: "Grassy Seed",
    },
    &ItemMetadataPastGen {
        id: 588,
        modern_id: Some(883),
        name: "Misty Seed",
    },
    &ItemMetadataPastGen {
        id: 589,
        modern_id: Some(882),
        name: "Psychic Seed",
    },
    &ItemMetadataPastGen {
        id: 590,
        modern_id: Some(564),
        name: "Normal Gem",
    },
    &ItemMetadataPastGen {
        id: 591,
        modern_id: Some(553),
        name: "Fighting Gem",
    },
    &ItemMetadataPastGen {
        id: 592,
        modern_id: Some(556),
        name: "Flying Gem",
    },
    &ItemMetadataPastGen {
        id: 593,
        modern_id: Some(554),
        name: "Poison Gem",
    },
    &ItemMetadataPastGen {
        id: 594,
        modern_id: Some(555),
        name: "Ground Gem",
    },
    &ItemMetadataPastGen {
        id: 595,
        modern_id: Some(559),
        name: "Rock Gem",
    },
    &ItemMetadataPastGen {
        id: 596,
        modern_id: Some(558),
        name: "Bug Gem",
    },
    &ItemMetadataPastGen {
        id: 597,
        modern_id: Some(560),
        name: "Ghost Gem",
    },
    &ItemMetadataPastGen {
        id: 598,
        modern_id: Some(563),
        name: "Steel Gem",
    },
    &ItemMetadataPastGen {
        id: 599,
        modern_id: Some(548),
        name: "Fire Gem",
    },
    &ItemMetadataPastGen {
        id: 600,
        modern_id: Some(549),
        name: "Water Gem",
    },
    &ItemMetadataPastGen {
        id: 601,
        modern_id: Some(551),
        name: "Grass Gem",
    },
    &ItemMetadataPastGen {
        id: 602,
        modern_id: Some(550),
        name: "Electric Gem",
    },
    &ItemMetadataPastGen {
        id: 603,
        modern_id: Some(557),
        name: "Psychic Gem",
    },
    &ItemMetadataPastGen {
        id: 604,
        modern_id: Some(552),
        name: "Ice Gem",
    },
    &ItemMetadataPastGen {
        id: 605,
        modern_id: Some(561),
        name: "Dragon Gem",
    },
    &ItemMetadataPastGen {
        id: 606,
        modern_id: Some(562),
        name: "Dark Gem",
    },
    &ItemMetadataPastGen {
        id: 607,
        modern_id: Some(715),
        name: "Fairy Gem",
    },
    &ItemMetadataPastGen {
        id: 608,
        modern_id: Some(645),
        name: "Ability Capsule",
    },
    &ItemMetadataPastGen {
        id: 609,
        modern_id: Some(565),
        name: "Health Wing",
    },
    &ItemMetadataPastGen {
        id: 610,
        modern_id: Some(566),
        name: "Muscle Wing",
    },
    &ItemMetadataPastGen {
        id: 611,
        modern_id: Some(567),
        name: "Resist Wing",
    },
    &ItemMetadataPastGen {
        id: 612,
        modern_id: Some(568),
        name: "Genius Wing",
    },
    &ItemMetadataPastGen {
        id: 613,
        modern_id: Some(569),
        name: "Clever Wing",
    },
    &ItemMetadataPastGen {
        id: 614,
        modern_id: Some(570),
        name: "Swift Wing",
    },
    &ItemMetadataPastGen {
        id: 615,
        modern_id: None,
        name: "Pretty Wing",
    },
    &ItemMetadataPastGen {
        id: 616,
        modern_id: Some(795),
        name: "Bottle Cap",
    },
    &ItemMetadataPastGen {
        id: 617,
        modern_id: Some(796),
        name: "Gold Bottle Cap",
    },
    &ItemMetadataPastGen {
        id: 618,
        modern_id: Some(853),
        name: "Red Nectar",
    },
    &ItemMetadataPastGen {
        id: 619,
        modern_id: Some(854),
        name: "Yellow Nectar",
    },
    &ItemMetadataPastGen {
        id: 620,
        modern_id: Some(855),
        name: "Pink Nectar",
    },
    &ItemMetadataPastGen {
        id: 621,
        modern_id: Some(856),
        name: "Purple Nectar",
    },
    &ItemMetadataPastGen {
        id: 622,
        modern_id: Some(492),
        name: "Fast Ball",
    },
    &ItemMetadataPastGen {
        id: 623,
        modern_id: Some(493),
        name: "Level Ball",
    },
    &ItemMetadataPastGen {
        id: 624,
        modern_id: Some(494),
        name: "Lure Ball",
    },
    &ItemMetadataPastGen {
        id: 625,
        modern_id: Some(495),
        name: "Heavy Ball",
    },
    &ItemMetadataPastGen {
        id: 626,
        modern_id: Some(496),
        name: "Love Ball",
    },
    &ItemMetadataPastGen {
        id: 627,
        modern_id: Some(497),
        name: "Friend Ball",
    },
    &ItemMetadataPastGen {
        id: 628,
        modern_id: Some(498),
        name: "Moon Ball",
    },
    &ItemMetadataPastGen {
        id: 629,
        modern_id: Some(499),
        name: "Sport Ball",
    },
    &ItemMetadataPastGen {
        id: 630,
        modern_id: Some(851),
        name: "Beast Ball",
    },
    &ItemMetadataPastGen {
        id: 631,
        modern_id: Some(576),
        name: "Dream Ball",
    },
    &ItemMetadataPastGen {
        id: 632,
        modern_id: Some(631),
        name: "Oval Charm",
    },
    &ItemMetadataPastGen {
        id: 633,
        modern_id: Some(632),
        name: "Shiny Charm",
    },
    &ItemMetadataPastGen {
        id: 634,
        modern_id: Some(118),
        name: "Burn Drive",
    },
    &ItemMetadataPastGen {
        id: 635,
        modern_id: Some(116),
        name: "Douse Drive",
    },
    &ItemMetadataPastGen {
        id: 636,
        modern_id: Some(117),
        name: "Shock Drive",
    },
    &ItemMetadataPastGen {
        id: 637,
        modern_id: Some(119),
        name: "Chill Drive",
    },
    &ItemMetadataPastGen {
        id: 638,
        modern_id: Some(904),
        name: "Fist Memory",
    },
    &ItemMetadataPastGen {
        id: 639,
        modern_id: Some(905),
        name: "Sky Memory",
    },
    &ItemMetadataPastGen {
        id: 640,
        modern_id: None,
        name: "Toxic Memory",
    },
    &ItemMetadataPastGen {
        id: 641,
        modern_id: Some(907),
        name: "Earth Memory",
    },
    &ItemMetadataPastGen {
        id: 642,
        modern_id: Some(908),
        name: "Rock Memory",
    },
    &ItemMetadataPastGen {
        id: 643,
        modern_id: Some(909),
        name: "Bug Memory",
    },
    &ItemMetadataPastGen {
        id: 644,
        modern_id: Some(910),
        name: "Ghost Memory",
    },
    &ItemMetadataPastGen {
        id: 645,
        modern_id: Some(911),
        name: "Steel Memory",
    },
    &ItemMetadataPastGen {
        id: 646,
        modern_id: Some(912),
        name: "Fire Memory",
    },
    &ItemMetadataPastGen {
        id: 647,
        modern_id: Some(913),
        name: "Water Memory",
    },
    &ItemMetadataPastGen {
        id: 648,
        modern_id: Some(914),
        name: "Grass Memory",
    },
    &ItemMetadataPastGen {
        id: 649,
        modern_id: Some(915),
        name: "Zap Memory",
    },
    &ItemMetadataPastGen {
        id: 650,
        modern_id: Some(916),
        name: "Psychic Memory",
    },
    &ItemMetadataPastGen {
        id: 651,
        modern_id: Some(917),
        name: "Ice Memory",
    },
    &ItemMetadataPastGen {
        id: 652,
        modern_id: Some(918),
        name: "Dragon Memory",
    },
    &ItemMetadataPastGen {
        id: 653,
        modern_id: Some(919),
        name: "Dark Memory",
    },
    &ItemMetadataPastGen {
        id: 654,
        modern_id: Some(920),
        name: "Fairy Memory",
    },
    &ItemMetadataPastGen {
        id: 655,
        modern_id: Some(483),
        name: "Rainbow Wing",
    },
    &ItemMetadataPastGen {
        id: 656,
        modern_id: Some(482),
        name: "Silver Wing",
    },
    &ItemMetadataPastGen {
        id: 657,
        modern_id: Some(943),
        name: "N-Solarizer",
    },
    &ItemMetadataPastGen {
        id: 658,
        modern_id: Some(944),
        name: "N-Lunarizer",
    },
    &ItemMetadataPastGen {
        id: 659,
        modern_id: Some(857),
        name: "Sun Flute",
    },
    &ItemMetadataPastGen {
        id: 660,
        modern_id: Some(858),
        name: "Moon Flute",
    },
    &ItemMetadataPastGen {
        id: 661,
        modern_id: Some(616),
        name: "Light Stone",
    },
    &ItemMetadataPastGen {
        id: 662,
        modern_id: Some(617),
        name: "Dark Stone",
    },
    &ItemMetadataPastGen {
        id: 663,
        modern_id: Some(584),
        name: "Relic Copper",
    },
    &ItemMetadataPastGen {
        id: 664,
        modern_id: Some(585),
        name: "Relic Silver",
    },
    &ItemMetadataPastGen {
        id: 665,
        modern_id: Some(586),
        name: "Relic Gold",
    },
    &ItemMetadataPastGen {
        id: 666,
        modern_id: Some(587),
        name: "Relic Vase",
    },
    &ItemMetadataPastGen {
        id: 667,
        modern_id: Some(588),
        name: "Relic Band",
    },
    &ItemMetadataPastGen {
        id: 668,
        modern_id: Some(589),
        name: "Relic Statue",
    },
    &ItemMetadataPastGen {
        id: 669,
        modern_id: Some(590),
        name: "Relic Crown",
    },
    &ItemMetadataPastGen {
        id: 670,
        modern_id: Some(776),
        name: "Normalium Z",
    },
    &ItemMetadataPastGen {
        id: 671,
        modern_id: Some(782),
        name: "Fightinium Z",
    },
    &ItemMetadataPastGen {
        id: 672,
        modern_id: Some(785),
        name: "Flyinium Z",
    },
    &ItemMetadataPastGen {
        id: 673,
        modern_id: Some(783),
        name: "Poisonium Z",
    },
    &ItemMetadataPastGen {
        id: 674,
        modern_id: Some(784),
        name: "Groundium Z",
    },
    &ItemMetadataPastGen {
        id: 675,
        modern_id: Some(788),
        name: "Rockium Z",
    },
    &ItemMetadataPastGen {
        id: 676,
        modern_id: Some(787),
        name: "Buginium Z",
    },
    &ItemMetadataPastGen {
        id: 677,
        modern_id: Some(789),
        name: "Ghostinum Z",
    },
    &ItemMetadataPastGen {
        id: 678,
        modern_id: Some(792),
        name: "Steelium Z",
    },
    &ItemMetadataPastGen {
        id: 679,
        modern_id: Some(777),
        name: "Firium Z",
    },
    &ItemMetadataPastGen {
        id: 680,
        modern_id: Some(778),
        name: "Waterium Z",
    },
    &ItemMetadataPastGen {
        id: 681,
        modern_id: Some(780),
        name: "Grassium Z",
    },
    &ItemMetadataPastGen {
        id: 682,
        modern_id: Some(779),
        name: "Electrium Z",
    },
    &ItemMetadataPastGen {
        id: 683,
        modern_id: Some(786),
        name: "Psychium Z",
    },
    &ItemMetadataPastGen {
        id: 684,
        modern_id: Some(781),
        name: "Icium Z",
    },
    &ItemMetadataPastGen {
        id: 685,
        modern_id: Some(790),
        name: "Dragonium Z",
    },
    &ItemMetadataPastGen {
        id: 686,
        modern_id: Some(791),
        name: "Darkinium Z",
    },
    &ItemMetadataPastGen {
        id: 687,
        modern_id: Some(793),
        name: "Fairium Z",
    },
    &ItemMetadataPastGen {
        id: 688,
        modern_id: Some(803),
        name: "Alorichium Z",
    },
    &ItemMetadataPastGen {
        id: 689,
        modern_id: Some(798),
        name: "Decidium Z",
    },
    &ItemMetadataPastGen {
        id: 690,
        modern_id: Some(805),
        name: "Eevium Z",
    },
    &ItemMetadataPastGen {
        id: 691,
        modern_id: Some(799),
        name: "Incinium Z",
    },
    &ItemMetadataPastGen {
        id: 692,
        modern_id: Some(926),
        name: "Kommonium Z",
    },
    &ItemMetadataPastGen {
        id: 693,
        modern_id: Some(922),
        name: "Lunalium Z",
    },
    &ItemMetadataPastGen {
        id: 694,
        modern_id: Some(925),
        name: "Lycanium Z",
    },
    &ItemMetadataPastGen {
        id: 695,
        modern_id: Some(802),
        name: "Marshadium Z",
    },
    &ItemMetadataPastGen {
        id: 696,
        modern_id: Some(806),
        name: "Mewnium Z",
    },
    &ItemMetadataPastGen {
        id: 697,
        modern_id: Some(924),
        name: "Mimikium Z",
    },
    &ItemMetadataPastGen {
        id: 698,
        modern_id: Some(794),
        name: "Pikanium Z",
    },
    &ItemMetadataPastGen {
        id: 699,
        modern_id: None,
        name: "Pikasunium Z",
    },
    &ItemMetadataPastGen {
        id: 700,
        modern_id: Some(800),
        name: "Primarium Z",
    },
    &ItemMetadataPastGen {
        id: 701,
        modern_id: Some(804),
        name: "Snorlium Z",
    },
    &ItemMetadataPastGen {
        id: 702,
        modern_id: Some(921),
        name: "Solganium Z",
    },
    &ItemMetadataPastGen {
        id: 703,
        modern_id: Some(801),
        name: "Tapunium Z",
    },
    &ItemMetadataPastGen {
        id: 704,
        modern_id: Some(1119),
        name: "Eject Pack",
    },
    &ItemMetadataPastGen {
        id: 705,
        modern_id: Some(1122),
        name: "Room Service",
    },
    &ItemMetadataPastGen {
        id: 706,
        modern_id: Some(1121),
        name: "Blunder Policy",
    },
    &ItemMetadataPastGen {
        id: 707,
        modern_id: Some(1120),
        name: "Heavy Duty Boots",
    },
    &ItemMetadataPastGen {
        id: 708,
        modern_id: Some(1123),
        name: "Utility Parasol",
    },
    &ItemMetadataPastGen {
        id: 709,
        modern_id: Some(1118),
        name: "Throat Spray",
    },
    &ItemMetadataPastGen {
        id: 710,
        modern_id: Some(1103),
        name: "Rusted Sword",
    },
    &ItemMetadataPastGen {
        id: 711,
        modern_id: Some(1104),
        name: "Rusted Shield",
    },
    &ItemMetadataPastGen {
        id: 712,
        modern_id: Some(1116),
        name: "Sweet Apple",
    },
    &ItemMetadataPastGen {
        id: 713,
        modern_id: Some(1117),
        name: "Tart Apple",
    },
    &ItemMetadataPastGen {
        id: 714,
        modern_id: Some(1253),
        name: "Cracked Pot",
    },
    &ItemMetadataPastGen {
        id: 715,
        modern_id: Some(1254),
        name: "Chipped Pot",
    },
    &ItemMetadataPastGen {
        id: 716,
        modern_id: Some(1109),
        name: "Strawberry Sweet",
    },
    &ItemMetadataPastGen {
        id: 717,
        modern_id: Some(1111),
        name: "Berry Sweet",
    },
    &ItemMetadataPastGen {
        id: 718,
        modern_id: Some(1110),
        name: "Love Sweet",
    },
    &ItemMetadataPastGen {
        id: 719,
        modern_id: Some(1112),
        name: "Clover Sweet",
    },
    &ItemMetadataPastGen {
        id: 720,
        modern_id: Some(1113),
        name: "Flower Sweet",
    },
    &ItemMetadataPastGen {
        id: 721,
        modern_id: Some(1115),
        name: "Ribbon Sweet",
    },
    &ItemMetadataPastGen {
        id: 722,
        modern_id: Some(1114),
        name: "Star Sweet",
    },
    &ItemMetadataPastGen {
        id: 723,
        modern_id: Some(1582),
        name: "Galarica Cuff",
    },
    &ItemMetadataPastGen {
        id: 724,
        modern_id: Some(1592),
        name: "Galarica Wreath",
    },
    &ItemMetadataPastGen {
        id: 725,
        modern_id: Some(1590),
        name: "Reins Of Uni",
    },
    &ItemMetadataPastGen {
        id: 726,
        modern_id: Some(1691),
        name: "Black Augurite",
    },
    &ItemMetadataPastGen {
        id: 727,
        modern_id: Some(1692),
        name: "Peat Block",
    },
    &ItemMetadataPastGen {
        id: 728,
        modern_id: None,
        name: "Hisui Rock",
    },
];
