...
describe('gen 5 conversion to bytes and back is lossless', async () => {
  const files = fs.readdirSync(pkmTestFilePath('pk5')).filter((f) => f.endsWith('.pkm'))
  await initializeWasm()

  for (const file of files) {
    const bytes = new Uint8Array(fs.readFileSync(pkmTestFilePath('pk5', file)))
    const original = PK5.fromBytes(bytes.buffer)
    const roundTrip = PK5.fromBytes(PK5.fromBytes(original.toBytes()).toBytes())
...
describe('gen 5 conversion to OHPKM and back is lossless', async () => {
  const files = fs.readdirSync(pkmTestFilePath('pk5')).filter((f) => f.endsWith('.pkm'))
  await initializeWasm()
