...
export default function SuggestedSaves(props: SaveFileSelectorProps) {
  ...
  const [
    { homeMons: homeMonMap, gen12: gen12LookupMap, gen345: gen345LookupMap },
  ...
  const openSavePaths = useMemo(
    () =>
      Object.fromEntries(
        Object.values(openSaves).map((save) => [save.filePath.raw, true])
      ),
    [openSaves]
  );
      ...
      E.match(
        ...
        async (possibleSaves) => {
          ...
          if (allPaths.length > 0) {
            const saves = (
              await Promise.all(allPaths.map((path) => loadSaveData(path)))
            ).filter(filterEmpty);
    ...
    (origin: number | undefined) =>
      origin
        ? getEnabledSaveTypes().find((s) =>
            s.includesOrigin(origin as GameOfOrigin)
          )
        : undefined,
    [getEnabledSaveTypes]
  );
    ...
    // },
    {
      ...
      renderCell: (params) => (
        <button
          ...
          disabled={
            params.row.tooEarlyToOpen ||
            params.row.filePath.raw in openSavePaths
          }
        >
          Open
        </button>
    ...
    },
    {
      ...
      renderValue: (value) => (
        <img
          ...
          height={40}
          src={getSaveLogo(
            saveTypeFromOrigin(value.origin),
            value.origin as GameOfOrigin
          )}
        />
      ),
