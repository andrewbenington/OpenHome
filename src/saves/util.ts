...
export function getMonSaveLogo(mon: PKMInterface, supportedSaves: SAVClass[]) {
  if (mon.pluginOrigin) {
    const pluginSave = supportedSaves.find(
      (s) => getPluginIdentifier(s) === mon.pluginOrigin
    );
  ...
  if (mon.gameOfOrigin === GameOfOrigin.ColosseumXD) {
    switch (
      colosseumOrXD(
        mon.dexNum,
        mon.ribbons?.includes("National Ribbon") || mon.isShadow
      )
    ) {
...
export function getSaveLogo(
  ...
  if (saveType?.prototype.getPluginIdentifier.call({})) {
    return getPublicImageURL(
      `logos/${saveType.prototype.getPluginIdentifier.call({})}.png`
    );
  }
  if (!origin) {
  ...
  return GameLogos[
    GameOfOriginData[origin]?.logo ??
      GameOfOriginData[origin]?.name.replace(" ", "") ??
      ""
  ];
}
