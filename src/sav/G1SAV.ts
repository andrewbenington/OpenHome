...
export class G1SAV extends SAV {
  ...
  constructor(path: string, bytes: Uint8Array) {
    ...
    if (this.currentPCBox < 6) {
      boxByteOffset = 0x4000 + this.currentPCBox * this.BOX_SIZE;
    } else {
      boxByteOffset = 0x6000 + (this.currentPCBox - 6) * this.BOX_SIZE;
    }
    console.log('currentbox', boxByteOffset, this.currentPCBox);
    this.bytes.set(
      this.bytes.slice(
...
export class G1Box implements Box {
  name: string;
