...
export class OhpkmV1 implements PKMInterface {
  static fromBytes(bytes: ArrayBuffer) {
    return new OhpkmV1(new Uint8Array(bytes))
  }
  ...
  constructor(arg: PKMInterface | PluginPKMInterface | OhpkmV1 | Uint8Array) {
    if (arg instanceof Uint8Array) {
      // If OhpkmV1 format has expanded, we want to increase the size of older files to
  ...
  public updateData(other: PKMInterface) {
    ...
    if (other.ability && !FORMATS_WITHOUT_ABILITIES.includes(other.format)) {
      // don't update if OhpkmV1 has hidden ability and the other mon is from
