...
import { AbilityIndex, Ball, Gender, Item, Languages, MetadataLookup, NatureIndex, ShinyLeaves, SpeciesLookup, } from '@pkm-rs/pkg';
...
function validDPLocation(index) {
...
export default class PK4 {
    ...
    gender;
    formeNum;
    shinyLeavesInner = new ShinyLeaves();
    ribbonBytes;
    gameOfOrigin;
    eggDate;
    metDate;
    ...
    metLocationIndex;
    eggLocationIndexDP;
    eggLocationIndexPtHGSS;
    metLocationIndexDP;
    metLocationIndexPtHGSS;
    ribbons;
    nickname;
    trainerName;
    trainerGender;
    ...
    constructor(arg, encrypted) {
        if (arg instanceof ArrayBuffer) {
            ...
            this.ribbons = byteLogic
                ...
                .concat(byteLogic.getFlagIndexes(dataView, 0x3c, 0, 32).map((index) => Gen4Ribbons[index + 28]))
                .concat(byteLogic.getFlagIndexes(dataView, 0x60, 0, 20).map((index) => Gen4Ribbons[index + 60]));
    ...
    toBytes(options) {
        ...
        byteLogic.setFlagIndexes(dataView, 0x24, 0, this.ribbons
            .map((ribbon) => Gen4Ribbons.indexOf(ribbon))
            .filter((index) => index > -1 && index < 28));
        byteLogic.setFlagIndexes(dataView, 0x3c, 0, this.ribbons
            .map((ribbon) => Gen4Ribbons.indexOf(ribbon) - 28)
            .filter((index) => index > -1 && index < 32));
        byteLogic.setFlagIndexes(dataView, 0x60, 0, this.ribbons
