...
export const getStandardPKMStats = (mon) => {
...
export const getGameBoyPKMStats = (mon) => {
    if (mon.dexNum < 1 || mon.dexNum > NationalDexMax) {
        return {
            hp: 0,
            atk: 0,
...
export const getStatGen3Onward = (mon, stat, level) => {
    if (mon.dexNum < 1 || mon.dexNum > NationalDexMax) {
        return 0;
    }
    const natureMultiplier = mon.nature.multiplierFor(stat);
    const metadata = MetadataLookup(mon.dexNum, mon.formeNum);
    const statAbbr = Stats.abbrLower(stat);
    if (metadata) {
        const baseStat = metadata.getBaseStat(stat);
        const iv = mon.ivs[statAbbr];
        const ev = mon.evs[statAbbr];
        return Math.floor(natureMultiplier * (Math.floor((level * (2 * baseStat + iv + Math.floor(ev / 4))) / 100) + 5));
    }
    return 0;
};
export const getHPGen3Onward = (mon, level) => {
    if (mon.dexNum < 1 || mon.dexNum > NationalDexMax) {
        return 0;
    }
...
export const getLevelGen12 = (dexNum, exp) => {
    if (dexNum > 251) {
        return 1;
    }
