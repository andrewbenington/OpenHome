import { NationalDex } from './NationalDex';
export const SPIKY_EAR = 1;
export const EXCLAMATION = 26;
export const QUESTION = 27;
export const HEAT = 1;
export const WASH = 2;
export const FROST = 3;
export const FAN = 4;
export const MOW = 5;
export const FAIRY = 17;
export const ORIGIN = 1;
export const SKY = 1;
export const THERIAN = 1;
export const RESOLUTE = 1;
export const ETERNAL_FLOWER = 5;
export const UNBOUND = 1;
export const TEN_PCT = 1;
export const TEN_PCT_PC = 2;
export const COMPLETE = 3;
export const DUSK_MANE = 1;
export const DAWN_WINGS = 2;
export const ULTRA = 3;
export const LGP_STARTER = 8;
export const LGE_STARTER = 1;
export const BLOOD_MOON = 1;
export const ORIGINAL = 1;
export const GRENINJA_ASH = 2;
export const BASE = 0;
export const ALOLA = 1;
export const GALAR = 1;
export const MAROWAK_ALOLA_TOTEM = 2;
export const MEGA_Z = 2;
export var SWEETS;
(function (SWEETS) {
    SWEETS[SWEETS["Strawberry"] = 0] = "Strawberry";
    SWEETS[SWEETS["Berry"] = 1] = "Berry";
    SWEETS[SWEETS["Love"] = 2] = "Love";
    SWEETS[SWEETS["Star"] = 3] = "Star";
    SWEETS[SWEETS["Clover"] = 4] = "Clover";
    SWEETS[SWEETS["Flower"] = 5] = "Flower";
    SWEETS[SWEETS["Ribbon"] = 6] = "Ribbon";
})(SWEETS || (SWEETS = {}));
export const NON_Z_MEGAS_LEGENDS_ZA = [
    36, 71, 121, 149, 154, 160, 227, 478, 485, 491, 500, 530, 545, 560, 604, 609, 652, 655, 658, 668,
    670, 687, 689, 691, 701, 718, 780, 870, 26, 26, 358, 359, 398, 445, 448, 623, 678, 678, 740, 768,
    801, 801, 952, 998, 978, 978, 978, 970,
];
export const Z_MEGAS_LEGENDS_ZA = [NationalDex.Absol, NationalDex.Garchomp, NationalDex.Lucario];
