...
use crate::pkm::{Ohpkm, Pkm};
use crate::resources::{AbilityIndex, MoveSlot};
...
impl Pkm for Pb7 {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        ...
        if size < Pb7::BOX_SIZE {
            return Err(format!(
                "byte array is too short for PB7 format (received {size}, expected >= 260)"
            ));
        }
