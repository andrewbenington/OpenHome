...
use crate::pkm::Pkm;
use crate::resources::{AbilityIndex, MoveSlot, OpenHomeRibbonSet};
...
impl Pkm for Ohpkm {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self, String> {
        let size = bytes.len();
        if size < Ohpkm::BOX_SIZE {
            return Err(format!(
                "byte array is too short for Ohpkm format (received {size}, expected >= 433)"
            ));
        }
        ...
        let mon = Ohpkm {
            ...
            ability_index: AbilityIndex::from(u16::from_le_bytes(
            ...
            ribbons: OpenHomeRibbonSet::from_bytes(bytes[56..72].try_into().unwrap()),
            ...
            height_absolute: f32::from_le_bytes(bytes[172..176].try_into().unwrap()),
            weight_absolute: f32::from_le_bytes(bytes[176..180].try_into().unwrap()),
            ...
            fullness: bytes[235],
            enjoyment: bytes[236],
            game_of_origin: bytes[237],
            game_of_origin_battle: bytes[238],
            country: bytes[239],
            region: bytes[240],
            ...
            egg_date: PokeDate::from_bytes(bytes[305..308].try_into().unwrap()),
    ...
    fn write_bytes(&self, bytes: &mut [u8]) {
        ...
        bytes[56..72].copy_from_slice(&self.ribbons.to_bytes());
        ...
        bytes[172..176].copy_from_slice(&self.height_absolute.to_le_bytes());
        bytes[176..180].copy_from_slice(&self.weight_absolute.to_le_bytes());
        ...
        bytes[237] = self.game_of_origin;
        bytes[238] = self.game_of_origin_battle;
        ...
        bytes[305..308].copy_from_slice(&self.egg_date.to_bytes());
