...
use crate::{
    pkm::{Pb7, Pkm},
    saves::{SaveDataTrait, lets_go::LetsGoSave},
};

const SAVES_DIR: &str = "save_files";

fn main() {
    load_and_print_save::<LetsGoSave>("lgpe_with_partner.bin");
...
fn load_and_print_save<SAVE: SaveDataTrait + Serialize>(filename: &str) {
    ...
    let mon = save.get_mon_at(0, 0).unwrap();
...
fn print_jolteon() {
    ...
    println!("{}", toml::to_string(&pb7).unwrap());
}
