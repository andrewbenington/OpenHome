...
use serde;
use crate::util::{PathData, parse_path_data};
...
pub fn recursively_find_desamume_saves(current_path: &Path, depth: usize) -> Result<Vec<PathData>, String> {
...
pub fn recursively_find_gambatte_saves(current_path: &Path, depth: usize) -> Result<Vec<PathData>, String> {
    if depth >= MAX_SEARCH_DEPTH {
        return Ok(vec![]);
    }
...
pub fn recursively_find_mgba_saves(current_path: &Path, depth: usize) -> Result<Vec<PathData>, String> {
    if depth >= MAX_SEARCH_DEPTH {
        return Ok(vec![]);
    }
