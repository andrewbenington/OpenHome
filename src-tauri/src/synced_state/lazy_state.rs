...
impl LazyState {
    pub(crate) fn from_partial_states(
        ...
        Self(Mutex::new(LazyStateInner {
            ...
            ohpkm_cache: SyncedStateWrapper(OhpkmCache::new()),
            lazy_state_change_list: SyncedStateWrapper(OhpkmCacheChanges { 0 : HashMap::new() }),
        }))
    }
}
...
impl LazyState {
    ...
    pub(crate) fn update_from_frontend(
        ...
        match state_identifier {
            ...
            LazyStateChange::ID => {
                let action: <OhpkmCache as SyncedState>::Action =
                    serde_json::from_value(action).map_err(|e| {
                        Error::unexpeted_condition_with_source(
                ...
                self.lock()?.ohpkm_cache.0.update(action);
            }
            _ => {
